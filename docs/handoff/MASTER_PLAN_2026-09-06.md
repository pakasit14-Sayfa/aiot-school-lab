...
| [x] | 1.3 | เคลียร์ test debt: Flutter fail 16 + pgTAP fail 6 | `2c31ecd` | ทุกตัวถูก แก้ / ลบ / ขึ้นทะเบียน known พร้อมเหตุผล — Flutter fail ล่าสุด (2026-09-08) เหลือ 8 (ทั้งหมดฝั่ง Executive ที่ Codex ทำอยู่ + 1 school_admin) re-count 2026-09-13: **pgTAP fail 0** (14/16 ที่ทดสอบ role `facility_manager` ที่ลบแล้ว → แทนด้วย `57_sensor_latest_and_device_list_tenant_scope`, `Result: PASS`) · Flutter fail 8 → 6 (เทสต์ปฏิทินนักเรียน 2 ตัวแดงเฉพาะเสาร์-อาทิตย์ แก้แล้ว; เหลือ 6 Executive ที่รอ seam `DirectorOverviewController`) · shared_core 1 (fixture ขาด `submission_version_id`) · **2026-09-16 `2c31ecd`: Flutter fail 0** — 4 ไฟล์ที่ assert mock 7 ก.ย. ถูกลบ, layout audit ใช้ seam, overflow หน้าฉุกเฉินแก้ · pgTAP PASS · shared_core 61/61 |
...
| D6 | ใครจัดนักเรียนเข้าวิชา — **เคาะแล้ว 2026-09-18**: แอดมินเพิ่มคน + จัดตารางเรียน (ห้อง×วิชา×ครู×คาบ) → นักเรียนในห้องเห็นวิชาอัตโนมัติ ไม่มีใครกดเพิ่มรายคน (ดู `DECISIONS_2026-09-18_enrollment_by_timetable.md`) | ยังไม่มี ticket — ต้องออกแบบก่อน (schema + RPC + หน้าแอดมินจัดตาราง) | เหลือถาม 3 ข้อ: เรียนข้ามห้องได้ไหม / ย้ายคอร์สเดิมยังไง / หน้าจัดตารางแอดมินหน้าตาแบบไหน |
