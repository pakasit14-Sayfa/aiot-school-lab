...
## 2026-09-23 — ไฟล์แนบในใบงาน: ครูแนบได้ นักเรียนเห็นแล้ว

**ทำไม** — `assignments` มี 11 คอลัมน์และไม่มีที่เก็บของแนบเลย ครูแนบเอกสาร
ประกอบใบงานไม่ได้มาตลอด ส่วนคลังความรู้ (`course_files`) เก็บได้แต่ไฟล์ที่อัป
ขึ้นถัง ลิงก์เก็บไม่ได้

**ออกแบบตามที่เทียบกับของจริง** — Google Classroom คัดลอกไฟล์ที่แนบเข้า class
Drive folder ให้เอง · Teams เก็บใน Class Files library ของ SharePoint ห้องนั้น
**ไม่มีเจ้าไหนถามครูว่าจะเก็บเข้าคลังไหม** และไม่มีเจ้าไหนบังคับให้บันทึกใบงาน
ก่อนถึงจะแนบได้ · ต่างจากสองเจ้าหนึ่งข้อตามที่เจ้าของสั่ง: ลิงก์เข้าคลังด้วย
(ของเขาแนบลิงก์ได้แต่ไม่เก็บเข้าคลัง เพราะลิงก์คัดลอกเข้า Drive ไม่ได้)

**หลังบ้าน** `20260923000000` + `20260923010000` — ขึ้น prod แล้ว
(`prod_apply_2026-09-23.sh` / `-23b.sh` verify ผ่าน 6/6)

- ตารางใหม่ `assignment_attachments` — ใบงานชี้ไปที่ของในคลัง ไม่ถือสำเนาเอง
  FK เป็น `on delete restrict` โดยตั้งใจ: ลบของในคลังที่ใบงานยังอ้างอยู่ไม่ได้
- `course_files` + `kind`/`url`/`category` · `storage_path` กับ `size_bytes`
  เป็น null ได้แล้วเพราะลิงก์ไม่มีไฟล์จริง มี CHECK คุมรูปร่างของแต่ละชนิด
- RPC ใหม่ 8 ตัว + แทนที่ `register_course_file` (รับ `p_category`) และ
  `list_course_files` (คืน `kind`/`url`/`category`)
- **ปิดบั๊กเก่า**: ชีตอัปโหลดในคลังความรู้มีช่อง "หมวด/บทเรียน" ให้กรอกมาตลอด
  แต่ RPC ไม่มีพารามิเตอร์รับ ค่าที่ครูพิมพ์หายทุกครั้งในโหมดจริง
- `delete_assignment` — ระบบไม่เคยลบใบงานได้เลย ใบที่มีคนส่งงานแล้วลบไม่ได้

**ฝั่งครู** `acf1895` — กริด 2 คอลัมน์ (แบบ ก ที่เลือกจาก 3 แบบ) มีพื้นที่แสดง
รูปจริง ช่องเพิ่มเป็นสมาชิกตัวสุดท้ายของกริด · ไฟล์ขึ้นหลังบ้านตอนกดบันทึก
ครั้งเดียวแบบปุ่ม Save/Discard ของ Teams กดยกเลิกจึงไม่เหลือทั้งใบงานร่างและ
ไฟล์ขยะ · "เอาออกจากใบงาน" ไม่ใช่ "ลบ" และบอกตรง ๆ ว่าไฟล์ยังอยู่ในคลัง

**ฝั่งนักเรียน** — บล็อก "เอกสารจากครู" ในชีตส่งงาน วางเหนือชุดข้อมูลเซนเซอร์
เพราะเป็นของที่ต้องอ่านก่อนลงมือ · ลิงก์เปิดตรง ไฟล์ขอ signed URL ก่อน ·
"โหลดไม่สำเร็จ" แยกจาก "ครูไม่ได้แนบอะไร" ชัดเจน ไม่งั้นนักเรียนจะทำงานส่งโดย
ขาดเอกสารที่ครูให้มาโดยไม่รู้ตัว

**ตรวจ** — analyze 0 error · test 984 ผ่าน (ไฟล์แนบฝั่งครู 8 · ฝั่งนักเรียน 4)
· ทุก RPC รันจริงบน local DB ด้วย session ครู/นักเรียน 10 ด่าน · เทียบชื่อ
พารามิเตอร์ที่ Dart ส่งกับลายเซ็นฟังก์ชันจริงครบ 10 ตัว · บนเครื่องจริงต่อ prod
สร้างใบงานพร้อมลิงก์สำเร็จ (`atts = 1`) แล้วเก็บของทดสอบออกจาก prod แล้ว

**ยังไม่ทำ** — UI หน้าคลังความรู้ (แสดงลิงก์ · หมวด · ป้าย "N ใบงาน" · เมนู
เปลี่ยนชื่อ/หมวด/ลบ) · ปุ่มลบใบงานในหน้าครู · ลากสลับลำดับไฟล์แนบ · แก้ URL
ของลิงก์ที่แนบไปแล้ว — RPC พร้อมหมดแล้วทุกข้อ เหลือแต่หน้าจอ

**กับดักที่เพิ่มใน PITFALLS** — ข้อ 11 (`do $$ create type $$` ไม่ทำงานผ่าน
`supabase db query`) และข้อ 12 (`create or replace` ไม่ทับฟังก์ชันที่จำนวน
พารามิเตอร์ต่างกัน)

**ต่อด้วยหน้าคลังความรู้** (`20260923020000` + UI) — คลังรับลิงก์แล้ว · หมวดที่
ครูกรอกถูกแสดงจริง (เดิมหน้าจอ hardcode `'เอกสารประกอบการเรียน'` ทุกไฟล์ ทั้งที่
ชีตอัปโหลดมีช่องให้กรอก) · ป้าย "N ใบงาน" บอกตั้งแต่ก่อนกดว่าลบไม่ได้ · เมนู ⋯
เปลี่ยนชื่อ/หมวด/แก้ลิงก์ปลายทาง/ลบ โดยเขียนกำกับว่าชื่อกับหมวดเป็นของตัวไฟล์
แก้ทีเดียวเปลี่ยนทุกใบงานที่ใช้อยู่

`list_course_files` คืน `used_by_assignments` นับจากฐานข้อมูล ไม่ใช่ให้แอปไล่
ดึงไฟล์แนบของทุกใบงานมานับเอง — วิธีหลังคือ N round trip ต่อการเปิดหน้าหนึ่งครั้ง

เทสต์ใหม่ 7 ตัว · `teacher_type_scale_test` จับได้ว่าป้ายใหม่ใช้ fontSize 10.5
ซึ่งไม่อยู่ในสเกล 8 ค่าของเลนครู แก้เป็น `TeacherType.caption` แล้ว (รวม 991 ผ่าน)

**ปิดงานไฟล์แนบ** — ลบใบงาน + สลับลำดับไฟล์แนบ (RPC มีบน prod อยู่แล้วตั้งแต่
`20260923000000` เหลือแค่ต่อหน้าจอ)

- **ลบใบงาน** อยู่ในเมนู ⋯ ของหน้ารายละเอียดเท่านั้น ไม่ใส่ในแถวรายการ — ต้อง
  เปิดเข้าไปเห็นก่อนว่ามีคนส่งกี่คนแล้วค่อยลบ และไปอยู่รวมกับ ขยายเวลาส่ง /
  ปิดรับงาน ซึ่งเป็นการกระทำระดับใบงานเหมือนกัน · ใบที่มีคนส่งแล้วปุ่มถูกปิด
  พร้อมบอกให้ใช้ปิดรับงานแทน และหลังบ้านเป็นด่านจริงอีกชั้น (จำนวนที่ส่งเปลี่ยน
  ได้ระหว่างเปิดหน้าค้างไว้)
- **สลับลำดับไฟล์แนบ** ใช้แตะค้างที่การ์ดแล้วขึ้นเมนู ไม่ทำ drag-and-drop —
  กริดเป็น Wrap การลากต้องเขียน Draggable/DragTarget เอง ทดสอบยากและชนกับการ
  เลื่อนหน้าฟอร์ม · สลับในเครื่องก่อน เขียนกลับตอนกดบันทึกเหมือนทุกอย่างในหน้านี้
  และยิง reorder เฉพาะตัวที่ลำดับเปลี่ยนจริง ไม่ใช่ทุกตัวทุกครั้ง

เทสต์ใหม่ 8 ตัว (ลบใบงาน 4 · สลับลำดับ 4) รวมทั้งโปรเจกต์ 999 ผ่าน

**ยังไม่ได้ตรวจบนซิม** — checkout หลักถูกอีกเซสชันใช้อยู่บน branch
`fix/super-admin-school-package-assertion` สลับ branch ไปตรวจไม่ได้
