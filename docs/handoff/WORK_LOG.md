...
## School Admin — ไล่ตรวจ 24 จุดที่ `state.sh` §5 flag ไว้ — 2026-09-25

§5 ของ `state.sh` grep หา 3 pattern ใน `pages/school_admin/*.dart`:
`catch (_) {}` (7 หน้า), fallback `?? '<ตัวเลข/ไทย>...'` (12 หน้า), และไม่มี
คำว่า `ยังไม่มีข้อมูล` เลย (5 หน้า) — รวม 24 จุด คาบเกี่ยว 20 หน้าไม่ซ้ำกัน (2
หน้าคือ `school_admin_dashboard_page`/`school_timetable_page` ตรวจไปแล้วใน
รอบก่อนหน้า สะอาดทั้งคู่) ไล่อ่านทั้งไฟล์ที่เหลือ 18 หน้าครบทุกจุดที่ถูก flag —
สคริปต์เตือนถูกว่า "ไม่ใช่ปลอมแน่นอน" จริง ๆ:

- **23/24 จุดเป็น false positive ทั้งหมด** — ส่วนใหญ่ (`catch (_) {}` ทั้ง 7
  จุด) คือ regex เจอโค้ดตัวอย่างที่ยกมาไว้ใน **คอมเมนต์บันทึกบั๊กเก่าที่ปิดไปแล้ว**
  ไม่ใช่โค้ดจริงที่รันอยู่ (catch จริงทุกจุดเป็น `catch (e)` ที่ debugPrint +
  ตั้ง error state จริง) ส่วน fallback `?? '...'` ที่เหลือแบ่งเป็น 2 กลุ่ม:
  ค่า default ของ dropdown filter ("ทั้งหมด"/"ทุกอาคาร" ไม่ใช่ข้อมูลจริง) และ
  placeholder ที่ใช้คำอื่นแทน (`'ไม่ระบุ'`/`'ยังไม่เคยรายงาน'`/`'ยังไม่ระบุตำแหน่ง'`)
  ส่วนที่ไม่มีคำว่า `ยังไม่มีข้อมูล` ทั้ง 5 หน้า ก็มี empty-state จริงแค่ใช้คำ
  ต่างกัน (`school_admin_attendance_settings_page`/`leave_approval`/
  `report_requirements` มีข้อความเฉพาะทางที่ชัดกว่าด้วยซ้ำ)
- **บั๊กจริง 1 จุด** — `school_admin_incident_inbox_page.dart` ไดอะล็อก
  รายละเอียดเหตุการณ์โชว์ `detail.severity` ดิบ (ค่าจริงเป็นอังกฤษ
  `'high'/'medium'/'low'` ตรงกับที่ `student_safety_page.dart`/
  `teacher_incident_inbox_page.dart` แปลเป็นไทยอยู่แล้ว) ไม่แปลเลย และตอน
  severity เป็น null ใส่ `'ปกติ'` ซึ่งฟังดูเหมือนมีคนประเมินแล้วจริง ๆ ทั้งที่
  ไม่มี แก้ให้แปลตรงกับ mapping เดียวกับหน้าอื่น (null → "เหตุเล็ก" ตาม
  convention เดิมของโปรเจกต์ที่ถือว่าไม่ระบุ = ต่ำสุด) commit `bcc8966`

**สรุปรวม 3 รอบตรวจ (Executive 15 + School Admin 24 หน้า + 24 จุด flag) —
รวม 2 บั๊กแสดงผลจริงทั้งเซสชันนี้** (`director_teachers_page` สีผิด,
`school_admin_incident_inbox_page` severity ดิบ) ไม่มีข้อมูลปลอมที่ไม่เปิดเผย
เลยแม้แต่จุดเดียวจากทั้งหมดที่ตรวจ — โปรเจกต์นี้อยู่ในสภาพที่ดีกว่าที่ signal
ดิบจาก `state.sh` §5 บอกไว้มาก แต่ก็ยืนยันด้วยว่ากฎ "grep พิสูจน์ได้แค่ว่าไม่
ปลอมแน่ ๆ ไม่เคยพิสูจน์ว่าไม่ปลอมเลย" ใช้ได้จริงทั้ง 2 ทาง — ทั้งจับผิดเกินไป
(false positive 23/24) และจับไม่ครบ (บั๊กจริงที่ regex ไม่ match เพราะ
`'ปกติ'`/`severity` ไม่ตรง pattern ที่ script เช็ค)
...
| ✅ หน้าแรกครูจอขาวทั้งหน้าตอนเปิดแอป — แก้ (2026-09-18) | เปิดแอปมือถือเป็นครูบน prod ครั้งแรกหลัง build `a0b3388` เจอจอขาวล้วน log บอก `Offset argument contained a NaN value` จาก `_UtilityGradientAreaChartPainter` (กราฟน้ำ-ไฟ) — ก่อนโหลดข้อมูลค่าทั้ง 5 วันเป็น 0 → `(v-min)/(max-min)` = 0/0 → NaN → `drawLine` assert → ทั้งเฟรมไม่วาด โดนครูทุกคนทุกครั้งที่เปิดแอป และถาวรถ้าโรงเรียนไม่มีค่าไฟ/น้ำสัปดาห์นั้น แก้ guard range/count-1 · `22e0dbe` |
| 📝 การตัดสินใจ D6: ใครจัดนักเรียนเข้าวิชา (2026-09-18) | ระหว่างทดสอบหน้านักเรียนด้วยข้อมูล prod พบว่าบัญชีเจ้าของอยู่ห้อง ม.1/1 แต่ไม่เห็นวิชาคณิต ม.1/1 เพราะห้องประจำชั้นกับสมาชิกคอร์สไม่เชื่อมกัน ครูต้องกดเพิ่มทีละคน เจ้าของเคาะว่าต้องเป็น **แอดมินเพิ่มคน + จัดตารางเรียน (ห้อง×วิชา×ครู×คาบ) → นักเรียนในห้องเห็นวิชาอัตโนมัติ ไม่มีใครกดเพิ่มรายคน** บันทึกเต็มใน `DECISIONS_2026-09-18_enrollment_by_timetable.md` · พบด้วยว่ารหัสเข้าร่วมคอร์สมีแค่ฝั่งครู ไม่มี RPC ให้นักเรียนใช้ (ฟีเจอร์ครึ่งเดียว) |
| ⚠️ พบระหว่างทดสอบครูบน iPhone — ยังไม่แก้ (2026-09-18) | เลนครูบนจอมือถือล้นขอบขวาหลายจุด (แถบปุ่มบนหน้ารายวิชา, ปุ่ม "สร้างกิจกรรม PBL" 150px, ป้าย "ผูกเซนเซอร์ AIoT" 39px) · ตัวเลขน่าสงสัยใน `teacher_courses_page`: "ส่งแล้ว 1/1 คน (100%)" ทั้งที่ `submissions` = 0, "อัตราเข้าเรียน 98.4%", "ความคืบหน้าสอน 87%", "3 กลุ่มเรียนหลัก", ตัวกรองห้อง ม.4/1 ม.4/2 ม.5/2 ที่ไม่มีในคอร์สจริง · hero หน้าแรกครูมีมาสคอต 2 ตัวซ้อน ข้อความโดนบัง · ปุ่มผูกชุดข้อมูล PBL-4 (`9258917`) ยังอยู่บน branch อีกบัญชี ไม่ได้ขึ้น main |
| ✅ D6 เฟส 1 — ฐานข้อมูล: แอดมินจัดตารางเรียน → นักเรียนเข้าวิชาตามห้องอัตโนมัติ (2026-09-20) | migration `20260919000000_admin_timetable_enrollment.sql`: ตาราง `teacher_subjects`, `school_periods` · `_class_room_key` · `sync_course_students_for_room/for_student` (internal, revoke จาก anon/authenticated) · `set_student_profile` + `import_school_users_batch_for_school_admin` เรียก sync · `create_course`/`enroll_student`/`set_class_schedule` เหลือ school_admin · `admin_set_room_timetable_slot` / `admin_clear_room_timetable_slot` / `list_room_timetable` / `list_school_classes` / `set|list_teacher_subjects` / `set|list_school_periods` · backfill คอร์สเดิม · pgTAP 68 26/26, ทั้งชุด 65 ไฟล์ 1,066 เคส PASS · สคริปต์ `scripts/prod_apply_2026-09-19.sh` รอเจ้าของรัน (runbook 4.8) · เขียนโดย agy (`9bd32dc`) ตรวจ+แก้โดยอีกเซสชัน |
| 🔎 สิ่งที่ตรวจพบในงาน agy เฟส 1 ก่อนขึ้น main (2026-09-20) | agy รายงานว่าเทสต์ผ่านหมด แต่ `npx supabase test db` จริงให้ **08_classroom_core ล้มทั้งไฟล์** (แก้เทสต์ให้ใช้ token นักเรียนสร้างคอร์ส) · `create_course` ใหม่ทำ check "term เป็นของโรงเรียนตัวเอง" หาย (แอดมินโรงเรียน A ดึงนักเรียนโรงเรียน B เข้าคอร์สได้) · `set_class_schedule` overload เก่า `(…,text,text)` ไม่ถูก drop → ทุก call 6 arg เป็น "not unique" และ client ที่ส่ง `p_period_type` ยังเรียกตัวเก่าที่ครูเขียนได้ · สคริปต์ prod ใช้ `db push` · ลบเทสต์ end_time<start_time ทิ้ง · ครูยังตั้งตารางได้ · **sync จับคู่ห้องแบบ string ตรงตัว** ทั้งที่ prod เก็บ profile (ม.1, '1') กับคอร์ส (ม.1, 'ม.1/1') → backfill บน prod จะได้ 0 — เพิ่ม `_class_room_key` · agy รายงานรอบสองว่า "แก้ครบ 6 ข้อ" ทั้งที่ branch ไม่มี commit ใหม่ และอ้าง RPC/trigger ที่ไม่มีอยู่จริง → ผู้ตรวจแก้เองทั้งหมด · ใบสั่งงานผิด 1 จุด: ชื่อ `list_school_rooms` ชนกับ RPC ห้องอาคารที่มีอยู่ agy เปลี่ยนเป็น `list_school_classes` ถูกต้อง |
| ⚠️ prod ได้ D6 เฟส 1 ฉบับก่อนตรวจ → migration ซ่อม `20260920000000` (2026-09-20) | เจ้าของรันสคริปต์ 09-19 ฉบับแรก (`db push`) จาก checkout ที่ยังเป็น `811d8c9` ก่อนที่ `5beea08` จะขึ้น main → prod มี overload ซ้อน 2 ตัว, ไม่มี `_class_room_key`, `auto_enrolled = 0` (อ่านจาก prod ยืนยัน) · เขียน `20260920000000_d6_phase1_prod_repair.sql` = นิยามฉบับตรวจแล้วทั้งชุดแบบ idempotent + drop overload ของ 9bd32dc + backfill ใหม่ · จำลองลำดับ prod ใน transaction บน local ผ่าน · สคริปต์ `prod_apply_2026-09-20.sh` (runbook 4.9) รอเจ้าของรัน · สคริปต์ 09-19 เปลี่ยนเป็น stub exit 1 |
| ✅ prod ซ่อม D6 เฟส 1 แล้ว (2026-09-20) | เจ้าของรัน `prod_apply_2026-09-20.sh`: overload 1 · `_class_room_key` มี · `create_course` เช็คโรงเรียน · `auto_enrolled = 2` (นักเรียน ม.1/1 3 คน: `student@` ถูกครูเพิ่มไว้เดิมจึงไม่นับ, `dashboard-viewer@` + บัญชีเจ้าของเข้าอัตโนมัติ) — ตรงตามที่ควร ผู้ตรวจคาดผิดเองว่า 3 |
| ✅ main build ไม่ผ่านตั้งแต่ agy push เฟส 2 — แก้ (2026-09-20) | `timetable_model.dart` มีปีกกาเกินปิด class ก่อน `operator==` · `timetable_service.dart` import `supabase_core.dart` ที่ไม่มีอยู่ และดึง token จาก `client.auth.currentSession` (Supabase Auth — ผิดกฎข้อ 1 จะส่ง token ว่างทุกครั้ง) · เรียก `list_school_rooms` (RPC ห้องอาคาร) ส่งปีการศึกษา null · เขียน service ใหม่ตาม pattern `chart_service` (`supabase` + `AuthService.sessionToken`, `list_school_classes` + ปีปัจจุบันจาก `list_academic_years`) · ลบ `patch_page.dart` ที่ root · `1fb6fee`, `15557ea` · **เฟส 2 ยังไม่ผ่านการตรวจ**: controller เรียก `list_teacher_subjects` แบบไม่ส่ง teacher_id ในฐานะแอดมิน → RPC คืน forbidden, มี `shared_ui/widgets/data_grid.dart` ใหม่ทั้งที่ควรใช้ pattern School Admin เดิม, ปนไฟล์ครู 3 ไฟล์ |
| ✅ ทดสอบหน้านักเรียนด้วยข้อมูล prod จริงบน iPhone — รอบแรก (2026-09-20) | หลัง D6: บัญชีเจ้าของเห็นวิชาคณิต ม.1/1 เอง (รายวิชา 1, ใบงาน 2 "2 งานค้าง") · sheet ส่งงานแสดงชุดข้อมูลที่ครูปัก (PBL-6) · หน้าชุดข้อมูล/สร้างกราฟ (PBL-7) แสดง "ไม่มีข้อมูลในช่วงเวลานี้" อย่างซื่อสัตย์ เพราะ (ก) เซนเซอร์ห้องทดลองไม่ส่งค่าตั้งแต่ 2–15 ก.ย. และ (ข) ครูปักโดยไม่มีช่วงเวลา → default 24 ชม.ล่าสุด · **พบช่องว่าง PBL-4 UI**: dialog ผูกชุดข้อมูล (`teacher_assignment_editor_page._openLinkSensorDialog`) ไม่มีช่องเลือกช่วงเวลา ทั้งที่ RPC รับ `p_time_start/p_time_end` → ปักข้อมูลย้อนหลังไม่ได้เลย · บั๊กเวลา: ใบงานแสดงกำหนดส่ง "02:24 น." = UTC ไม่แปลงเป็นเวลาไทย (09:24) · การ์ด AIoT หน้าแรกบอก "อัปเดต 19 วันที่แล้ว" ทั้งที่ dashboard บอก 5 วัน (คนละ metric) — ควรใช้ค่าล่าสุดของทุก metric · เจ้าของเคาะ D7: นักเรียนไม่ต้อง OTP |
| ✅ PBL-4 ขึ้น main + ช่วงเวลาในหน้าปักชุดข้อมูล (2026-09-20) | cherry-pick `9258917` (UI ปักชุดข้อมูลของอีกบัญชี ที่ค้างอยู่บน branch) + เพิ่มปุ่มลัด 24 ชม./7 วัน/30 วัน/กำหนดเอง และ picker เริ่ม/สิ้นสุด ส่ง `timeStart/timeEnd` ถึง `link_assignment_sensor_dataset` · dialog scroll ได้บนมือถือ · เทสต์ 3/3 · `1a7269f` · ครู (บัญชีเจ้าของ) ปัก "เซนเซอร์ห้องทดลอง · temperature · 21/8–20/9" บน prod ผ่านแอปจริง |
| ✅ นักเรียนเห็นข้อมูลเซนเซอร์จริงบน iPhone ครั้งแรก (2026-09-20) | ปิด "ข้อ 1" ที่ค้างตั้งแต่ 18 ก.ย.: ใบงาน → ชุดข้อมูล → กราฟอุณหภูมิ ต่ำสุด 24.3 / เฉลี่ย 26.5 / สูงสุด 28.8 °C มีเส้นจริง · กราฟของฉัน (PBL-7) สร้าง+บันทึก+เปิดดูซ้ำได้ (มี 1 กราฟบน prod) · **พบ 2 ข้อควรแก้:** (1) `get_sensor_history` ตัดที่ 1,000 จุด → ช่วง 30 วันเห็นแค่ 29/8 09:48–10:37 (49 นาที) ต้อง bucket/downsample ฝั่ง server ตามความยาวช่วง (2) แกน Y ของ `_DatasetChart` ป้าย "28.8"/"24.3" ตัดขึ้นสองบรรทัด — reservedSize แคบไป · ออกจากระบบฝั่งนักเรียนไม่มีกล่องยืนยันเหมือนครู · เจ้าของแจ้งว่าเครื่องนี้ trusted 30 วันจึงไม่ต้อง OTP |
| ✅ sensor_history ย่อข้อมูลตามช่วง + แกนกราฟ (2026-09-20) | สาเหตุ 1,000 จุดคือ PostgREST `max_rows` ไม่ใช่ RPC → `20260920020000`: นับแถวก่อน ถ้าเกิน `p_max_points` (default 1000) ให้ bucket เฉลี่ยโดยความกว้าง = ช่วงข้อมูลที่มีจริง/(max-1) (ไม่ใช่ช่วงที่ขอ จึงไม่เสียความละเอียดเมื่อปักยาวแต่ข้อมูลสั้น) · ต่ำกว่า max คืนค่าดิบ · pgTAP 69 8/8 (3,000 แถว/2.5 ชม. → ≤1000 จุดครบทั้งช่วง; max 100; ดิบไม่ถูกเฉลี่ย; ว่างคืน 0) · `AiotLabService.getSensorHistory` รับ `maxPoints` · `_DatasetChart`: แกน Y reservedSize 40→46 + label บรรทัดเดียว, แกน X ใส่วันที่เมื่อช่วง > 24 ชม. · สคริปต์ `prod_apply_2026-09-20b.sh` (runbook 4.10) รอเจ้าของรัน · หมายเหตุ: `iphone_layout_test` เคส `school_timetable_page` ของ agy ล้มอยู่บน main (ล้น 1.2px) ก่อนงานนี้ — ของเฟส 2 |
| ✅ prod ใช้ sensor_history ใหม่แล้ว (2026-09-20) | เจ้าของรัน `prod_apply_2026-09-20b.sh` · iPhone: ชุดข้อมูลอุณหภูมิ 21/8–20/9 แสดง **135 จุด ครบ 29/8–2/9** (เดิม 1,000 จุด = 49 นาที) ค่าจริง 23.6–32.8 °C · ข้อมูล prod เป็นช่วง ๆ ไม่ต่อเนื่อง (bucket ~6 นาที มีข้อมูลแค่ 135 ช่อง) — สอดคล้องกับเซนเซอร์ที่ขาด ๆ หาย ๆ |
| ✅ D6 เฟส 2 ตรวจ+แก้ให้ใช้ได้จริง — หน้าจัดตารางเรียนแอดมิน (2026-09-20) | ของ agy (`7e271aa`..`811d8c9`) ที่ push main เองก่อนตรวจ: **เปิดไม่ขึ้นเลย** เพราะเรียก `list_teacher_subjects` แบบไม่ส่งครู → RPC คืน forbidden; empty state กด "ไปตั้งค่าคาบเวลา" → `pushNamed('/school_admin/settings')` ที่ไม่มี route → crash; **ไม่มี UI ตั้งคาบเวลาที่ไหนเลย** (settings page ไม่ถูกแตะ) ทำให้หน้าติดอยู่ที่ "ยังไม่ได้ตั้งค่าคาบเวลา" ตลอดกาล; ไม่มี UI วิชาที่ครูสอนในหน้าครู; ครู "ไม่มี" ได้ทั้งที่ RPC บังคับ (จะได้ error ดิบ `teacher_required`); error ทุกจุดโชว์ `$e` ดิบ; `ClassSchedule.fromJson` ไม่อ่าน `teacher_id` (แก้ช่องเดิมแล้วครูหาย); เทอม default = ตัวแรกของ list; `data_grid.dart` ใน shared_ui ไม่มีใครใช้; ล้น 1.2px บนมือถือ (dropdown) · **แก้:** migration `20260920030000` (admin+null = ทุกครูพร้อมชื่อ · slot บังคับครู + จำคู่ครู↔วิชาเข้า `teacher_subjects` เองแทนหน้ากรอกแยก) · sheet ตั้งคาบเวลาในหน้าเดียวกัน (ค่าตั้งต้น 8 คาบ 08:30 คาบละ 50 นาที ไม่เขียนจนกด บันทึก · ตรวจเวลาทับ/ย้อน) · ครูบังคับใน sheet · `thaiError()` แปลงทุก code · ปุ่มลองใหม่ · เทอม default = เทอมที่ครอบวันนี้ · isExpanded · ลบ data_grid · เทสต์ connection 3 เคส + layout ผ่าน · สคริปต์ `prod_apply_2026-09-20c.sh` (runbook 4.11) · **ยังไม่ทำ:** ช่อง "วิชาที่สอน" ในหน้าครูของแอดมิน (ตอนนี้ตารางเป็นคนสอนให้ระบบจำแทน) |
| ✅ หน้าจัดตารางแอดมินบน iPhone — เมนูเข้าไม่ถึง + RPC 42804 (2026-09-20) | เปิดเป็นแอดมินจริงบน prod: (1) กริดเมนูหน้าแรกมือถือเป็นลิสต์แยกจาก sidebar หยุดที่หน้า 19 → หน้า 20–24 (รวมจัดตารางเรียน) ไม่มีทางเข้าบนมือถือเลย เพิ่มครบ `040e3ae` (2) โหลดหน้าแล้ว "โหลดตารางเรียนไม่สำเร็จ" → log: `list_school_classes` 42804 varchar vs text (agy เขียน, ไม่มี pgTAP เรียก) → migration `20260920040000` cast + เพิ่ม 5 เคส execution guard ทุก RPC ที่หน้านี้เรียก · สคริปต์ `prod_apply_2026-09-20d.sh` (runbook 4.12) |
| ✅ แอดมินจัดตารางจริงบน prod ครั้งแรก + บั๊กอ่านกลับ (2026-09-20) | ตั้งคาบเวลา 8 คาบ (sheet ใหม่) ✅ → จัดคาบ จันทร์/คาบ 1 = คณิตศาสตร์ · ครู ทดสอบ ✅ เขียนลง prod จริง (`class_schedules` 15:04Z, ผูกกับคอร์สคณิต ม.1/1 เดิมผ่าน `_class_room_key`) **แต่กริดไม่แสดง**: `list_room_timetable`/`admin_clear_room_timetable_slot` ยังเทียบห้องตรงตัว → `20260920050000` + pgTAP 5 เคส (40/40) · sheet เลือกครูเคยโชว์แอดมิน/ผอ. ด้วย (RPC จะปฏิเสธ teacher_not_found) → กรองเฉพาะ role teacher · สคริปต์ `prod_apply_2026-09-20e.sh` (runbook 4.13) · เกร็ด: แป้นจำลองเป็นไทย พิมพ์ "คณิตศาสตร์" ผ่านทูลต้องส่งคีย์ Kedmanee `8Ib9Lkl9iN` |
| ✅ D6 ครบวงจรบน prod (2026-09-20 22:10) | หลัง 4.13 กริดจัดตารางแสดง จันทร์/คาบ 1 = คณิตศาสตร์ · ครู ทดสอบ บน iPhone · วันนี้ prod ได้ migration 6 ตัว (4.8–4.13) ทั้งหมดผ่านสคริปต์ที่เจ้าของรันเอง · เหลือของ D6 ที่ยังไม่ทำ: เฟส 3 ซ่อนปุ่มสร้างคอร์ส/เพิ่มนักเรียน/รหัสเข้าร่วมฝั่งครู (RPC ปฏิเสธแล้วแต่ปุ่มยังอยู่) · ช่อง "วิชาที่สอน" ในหน้าครูของแอดมิน (ระบบจำจากตารางแทนไปก่อน) · ปิด D6 ใน MASTER_PLAN §4 |
| ✅ ตารางเรียน v2 — เจ้าของติว่าแบบเดิม "ยาก ไม่สวย ไม่สากล" (2026-09-20/21) | เจ้าของกำหนดโครง: **ทั้งโรงเรียน → ม.ต้น/ม.ปลาย → ห้อง** · ทำใหม่ทั้งหน้าในไฟล์เดิม `school_timetable_page.dart` (ไม่มี route ใหม่): ภาพรวม = ห้องทุกห้องของปีแยกมัธยมต้น/ปลาย/อื่น ๆ พร้อมแถบ จัดแล้ว x/y คาบ + tile ครูชนกัน (กดดูรายการ) · หน้าห้อง = ดูทีละวัน (chip จ.–ศ. + ปัดซ้ายขวา) คาบเป็นการ์ดสีตามวิชา (hash ชื่อ → 8 สี) พักเป็นแถบเทา ว่างเป็นกรอบประ · sheet ใส่วิชา: เลือกจาก "วิชาที่ห้องนี้เรียนอยู่" แตะเดียว (ครูติดมาด้วย) หรือวิชาใหม่ + ครูกรองจาก teacher_subjects + **เตือนครูชน** ก่อนบันทึก · เมนูห้อง: คัดลอกจากห้องอื่น / ใช้เทอมอื่น / ล้าง · คาบเวลามีพักกลางวัน (ค่าตั้งต้น 4+พัก+4) · DB `20260921000000` 6 RPC + pgTAP 70 20/20 · เทสต์หน้า 7 เคส + layout 360/375/390 · ยังไม่มี: พิมพ์ PDF, นำเข้า Excel · สคริปต์ `prod_apply_2026-09-21.sh` (runbook 4.14) |
| ✅ ตารางเรียน v2 ใช้จริงบน prod (2026-09-20 22:39) | เจ้าของรัน 4.14 · บน iPhone: ภาพรวม (มัธยมต้น · ม.1/1 · 3 คน · 1/40) → ห้อง → วันจันทร์ การ์ดคณิตสีเขียว → แตะคาบ 2 → sheet เสนอ "คณิตศาสตร์ · ครู ทดสอบ · 1 คาบ/สัปดาห์" แตะเดียว วิชา+ครูมาครบ → บันทึก → 2/40 · เกร็ดคอสเมติก: หัวเรื่องขึ้น "ภาคเรียนที่ 1/2569 (2569)" เพราะชื่อเทอมมีปีอยู่แล้ว |
| ✅ หน้าห้องเป็นปฏิทินสัปดาห์ (2026-09-20 23:xx) | เจ้าของ: "เอาเป็นแบบเหมือนปฏิทินดีกว่า" → แทนรายวันด้วยกริดสัปดาห์แบบ Google Calendar: แกนเวลาซ้าย 38pt · 5 คอลัมน์ จ.–ศ. (คอลัมน์วันนี้เน้น) · บล็อกสีตามวิชา 58pt (ชื่อ 2 บรรทัด + ครู) · พักเป็นแถบเทาข้ามทั้งสัปดาห์ · ช่องว่างกรอบบาง · legend สีวิชาล่าง · **แตะบล็อกที่มีวิชา → เมนูสั้น** (เปลี่ยน / วางวิชานี้ในคาบอื่น… / ลบ) แทน sheet เต็ม · แตะช่องว่าง = sheet แตะเดียวเหมือนเดิม · cell มี `ValueKey('slot-<day>-<period>')` ให้เทสต์ · ไม่แตะหลังบ้าน · เทสต์ 8 เคส + layout ผ่าน |
| ✅ D6 เฟส 3 — ฝั่งครูเป็นผู้อ่านตาราง (2026-09-21) | `teacher_courses_page`: ถอดปุ่ม "เพิ่มรายวิชาใหม่" (hero + app bar) / "คัดลอกรายวิชา" / "รหัสเข้าร่วม" / "เพิ่มนักเรียนเข้ารายวิชา" (RPC ปฏิเสธครูตั้งแต่ `20260919000000` ปุ่มเหลือไว้มีแต่ล้ม) แทนด้วยป้าย "รายวิชาและนักเรียนมาจากตารางเรียนที่แอดมินจัด" · ตัวเลขปลอมที่จดไว้ 18 ก.ย.: "3 กลุ่มเรียนหลัก" → จำนวนห้องจริง · "อัตราเข้าเรียน 98.4%" → ลงทะเบียนในวิชา · tile "ความคืบหน้าสอน 87%" ลบ (ไม่มีข้อมูล) · ตัวกรองห้อง ม.4/1…ม.6/3 → ห้องจริงจากคอร์ส · "ส่งแล้ว 1/1 คน (100%)" ลบ (list_assignments ไม่มีนับ) · `teacher_class_schedule_page` ถอดปุ่มเพิ่ม/ลบคาบ (เตรียมสอนคงไว้ — RPC คนละตัว) · method modal เก่า 5 ตัวเหลือเป็น unused (ลบทีหลังได้) · เทสต์ครู 9 + layout ผ่าน · **MASTER_PLAN D6 ปิดสมบูรณ์** เหลือช่อง "วิชาที่สอน" ในหน้าครูของแอดมินเป็น nice-to-have |
| ✅ บั๊กเล็ก 4 จุดจากการทดสอบ iPhone 20 ก.ย. (2026-09-21) | (1) กำหนดส่ง/ส่งแล้ว ในหน้าใบงานนักเรียนแสดงเวลา UTC (02:24 แทน 09:24) → `toLocal()` (2) การ์ด AIoT หน้าแรก "อัปเดต 19 วันที่แล้ว" ใช้แค่ pm25 → ใช้ค่าใหม่สุดของทุก metric (3) ออกจากระบบฝั่งนักเรียนไม่มีกล่องยืนยัน → เพิ่มเหมือนครู (เทสต์เดิมปรับให้กดยืนยันก่อน) (4) หัวหน้าจัดตาราง "ภาคเรียนที่ 1/2569 (2569)" → `Term.displayLabel` ไม่ต่อปีซ้ำ · เทสต์ 28 เคสผ่าน |
| ✅ S3 เลนครูบน iPhone — หน้ารายวิชา + 6 แท็บ ไม่ล้นจอ (2026-09-21) | เขียน probe pump `TeacherCoursesPage` + `TeacherCourseDetailPage` ทุกแท็บที่ 360/375/390/402 → พบ 7 จุด: หัวข้อ Row(spaceBetween)+ปุ่ม ล้นขวา 270–670px ในแท็บ นักเรียน/ใบงาน/กลุ่ม/คะแนน · การ์ดสถิติ 2 คอลัมน์ aspect 1.5 ตัดบรรทัด 3 (12–54px) · แถวห้องเรียนในการ์ดวิชาล้น 14px · summary card คะแนน aspect 3.0 ตัด 5–45px · แก้ด้วย `_SectionHeader` (ปุ่มตกบรรทัดเมื่อ < 480pt), สถิติเป็น IntrinsicHeight rows, ห้องเป็น Wrap, aspect 2.0 · probe กลายเป็น guard ถาวร `test/teacher_iphone_layout_test.dart` (7 เคส × 4 ขนาด) · hero มาสคอตซ้อนที่จดไว้ 18 ก.ย. ตรวจแล้วเป็นภาพจาก build เก่า — โค้ดแก้แล้วตั้งแต่ 18 ก.ย. |
| ✅ file_picker 8.1.6 → 11.0.3 — ปลดบล็อก Android build (2026-09-21) | ฝั่ง Windows (`2799a25`) รายงานว่า `flutter build apk` ล้ม: AAR ของ file_picker 8 คอมไพล์ที่ compileSdk 34 แต่ `flutter_plugin_android_lifecycle 2.0.35` (ผ่าน image_picker) บังคับ 36 → ต้องอัปเกรดแพ็กเกจที่ใช้ร่วมกันทั้งสองแพลตฟอร์ม ฝั่ง Mac ยืนยันแล้ว: `FilePicker.platform.x` → `FilePicker.x` 10 จุด/8 ไฟล์ · analyze 0 error · **iOS simulator build ผ่าน** · `flutter test` 909/909 · lock เพิ่ม args/dbus (transitive) · ฝั่ง Windows ให้ `git pull` แล้วรวม `compileSdk = 36` จาก branch `agent/android-round1` ต่อได้ |
| ✅ S3 ตรวจด้วยตาบน iPhone — เจอเพิ่ม 2 จุดที่ probe ไม่เห็นเพราะ mock ว่าง (2026-09-21) | แท็บใบงาน: แถว "กำหนดส่ง: 2026-09-01 02:24:39.686439Z" (UTC ดิบ) + ป้าย "ผูกเซนเซอร์ AIoT" ที่**ขึ้นทุกใบงานโดยไม่มีเงื่อนไข** ล้น 39px → วันที่ไทย พ.ศ. + ถอดป้าย · แท็บนักเรียน: ป้าย "🟢 เรียนปกติ (Active)" **คงที่ทุกแถว** (RPC ไม่มี status) ดันชื่อ/อีเมลล้น 28–60px → ถอด + ชื่อ/อีเมล ellipsis · guard เพิ่มนักเรียนจริง 1 คนใน mock · `1b1a6fb` + นี้ · **บั๊กใหม่ที่จด**: เลนแอดมินบนมือถือ**ไม่มีปุ่มออกจากระบบ** (มีแค่ "ออกทุกอุปกรณ์" ในโปรไฟล์ที่เข้าถึงได้เฉพาะ sidebar เดสก์ท็อป) → ทำใน S4 |
| ✅ หัวหน้ารายละเอียดวิชาครูบนมือถือ — แบบ B แถบสีเต็มจอ (2026-09-21) | เจ้าของสั่ง "ออกแบบมาใหม่" เสนอ 3 แบบ เลือก **B (Google Classroom)**: จอ < 900 ใช้ `CustomScrollView` + `SliverAppBar` pinned สี = `SubjectColor.of(ชื่อวิชา)` (ตรงกับตารางเรียนแอดมิน) · หัวขยาย 148pt ชื่อวิชาใหญ่ + ห้อง·จำนวนคน · ยุบเป็นแถบบาง ชื่อ fade-in · แท็บเส้นใต้บนแถบสี ติดจอ · การ์ด "งานรอตรวจ" / "คาบถัดไป" (คำนวณจาก `list_teacher_schedules` จริง) · ปุ่มจัดการกลุ่ม/ปิดวิชา → เมนู ⋯ · **ตัด** uuid, breadcrumb, การ์ดสีใหญ่, "ความคืบหน้าการสอน 0%" (ค่าคงที่), "ฉบับร่าง — ยังไม่เผยแพร่" · เดสก์ท็อป ≥ 900 ยังใช้ shell เดิม · seam `loadSchedules` · เทสต์ 15 ผ่าน |
| ✅ แท็บใบงานครู — รายการแบบ iOS (แบบ A) (2026-09-21) | เจ้าของสั่งออกแบบใหม่ เสนอ 3 แบบ เลือก A: แถวกะทัดรัด (จุดสถานะ · ชื่อ · "ส่ง 1 ก.ย. 09:24 · ส่งแล้ว 0/3 · เซนเซอร์ 2" · ลูกศร) แตะแถว → action sheet ตรวจงาน/แก้ไข แทน 2 ปุ่มต่อการ์ด · ชิปกรอง ทั้งหมด/รอตรวจ/ฉบับร่าง พร้อมตัวเลข แทน Draft/Published · ปุ่ม "สร้าง" เดียว → sheet ใบงาน/PBL · empty state แบบเชิญ · ตัดหัว "รายการใบงาน/ภารกิจ (n งาน)" ซ้ำ · **หลังบ้าน** `20260921010000`: `list_assignments` เพิ่ม `submitted_count/pending_grade_count/total_students/dataset_count` (เดิมไม่มี → ป้าย "1/1 (100%)" เก่าเลยเป็นค่าปลอม) pgTAP 11 +4 · `AssignmentSummary` รับ 4 ช่อง default 0 · สคริปต์ `prod_apply_2026-09-21b.sh` (runbook 4.15) |
| ✅ ฟลว์ตรวจงานครู 3 หน้าใหม่ (2026-09-21) | เจ้าของส่ง 3 ภาพ (action sheet / หน้าตรวจงาน / ฟอร์มแก้ไข) สั่ง "ออกแบบมาใหม่ ทั้งหมด" → เสนอ 3 หน้า เจ้าของ "แก้ มาเป็นแบบนี้เลย" · **(1) หน้าใบงาน** ใหม่ `teacher_assignment_detail_page.dart` แทน action sheet: AppBar สีวิชา · ป้ายสถานะ/กำหนดส่ง/งานกลุ่ม · 3 ตัวเลข ส่งแล้ว /N · รอตรวจ · ตรวจแล้ว จาก `list_course_students` × `list_submissions` · คำสั่ง · ชิปชุดข้อมูล · รายชื่อทุกคนในห้องพร้อมสถานะ ยังไม่ส่ง/รอตรวจ/ตรวจแล้ว · ปุ่ม ตรวจงาน / แก้ไข · เมนู ⋯ ขยายเวลาส่ง (date+time → `update_assignment`) / ปิดรับงาน / เผยแพร่ · **(2) หน้าตรวจงาน** `teacher_grading_page.dart` เขียนใหม่: ถอด 4 tile ด่วน/ปกติ/ส่งครบ/ร่าง + ปุ่มสร้าง · แถวแบบ iOS แยกหมวด เลยกำหนดส่ง → กำลังเปิดรับ → ฉบับร่าง (ปุ่ม เผยแพร่ ในแถว) · ชิป รอตรวจ n / เลยกำหนด n / ทั้งหมด n · meta "ม.1/1 · ส่ง x/y · รอตรวจ n · เลย d วัน" จากตัวเลข RPC จริง · ตัวกรองวิชาเป็น sheet · seam เหลือ 3 (listMyCourses/listAssignments/publishAssignment) · **(3) ฟอร์มใบงาน** ใหม่ `teacher_assignment_form_page.dart` (หน้าเต็ม iOS grouped: ยกเลิก/บันทึก บนหัว, กลุ่ม ข้อมูล / การส่งงาน / ชุดข้อมูลเซนเซอร์) แทน `_AssignmentFormSheet`: ตัดช่อง "ประเภทงาน" (worksheet เสมอ) · **กำหนดส่งเป็น DateTime จริงส่งถึง RPC** (sheet เดิมเก็บเป็นข้อความโชว์ ไม่เคยส่ง `p_due_at`) แสดง พ.ศ. · เกณฑ์การให้คะแนนเป็น picker · สวิตช์ "เผยแพร่ให้นักเรียน" → publish/unpublish ตอนบันทึก · ชุดข้อมูลแถวละ ชื่อ metric ไทย · อุปกรณ์ · ช่วงเวลา · ✕ ถอด (ยืนยันก่อน) + เพิ่มชุดข้อมูล sheet · ถามก่อนทิ้งการแก้ไข · **บั๊กที่เจอระหว่างทำ**: ปุ่มสร้างใบงานในแท็บวิชาเปิด modal โดย**ไม่ส่ง courseId** → สร้างใต้ `courses.first` (ผิดวิชาถ้าครูมี >1 วิชา) แก้ให้ส่งวิชาที่เปิดอยู่ · **หลังบ้าน** `20260921020000`: `unpublish_assignment` + `unlink_assignment_sensor_dataset` (ครูของวิชาเท่านั้น) pgTAP 11 +5 (24/24) · `AssignmentService` +2 · `AssignmentSummary.copyWith` · เทสต์: grading 5 เขียนใหม่ · detail 3 · form 4 · layout guard +3 หน้า (10 เคส × 4 ขนาด) · ครูทั้งชุด 143/143 · สคริปต์ `prod_apply_2026-09-21c.sh` (runbook 4.16 ⏳) · `_AssignmentFormSheet` เก่ายังอยู่ให้ desktop `TeacherAssignmentEditorPage` ใช้ (ลบทีหลังพร้อม modal เก่า) |
| ⚠️ env.prod.json หายจาก checkout หลัก (2026-09-21) | ตอน cold relaunch พบ `/Users/sayfa/my_first_app/env.prod.json` ไม่มี (น่าจะถูก**ย้าย**แทนคัดลอกตอนเจ้าของเอาไป Windows) · กู้จากสำเนาใน `/Users/sayfa/my_first_app_d6_phase1/` · ไฟล์นี้ gitignore ตั้งใจ — ถ้าหายอีกให้หาจาก worktree เก่าหรือขอเจ้าของ (ค่าคือ URL `smqoknnftgjyhrnzugar` + publishable key เท่านั้น) |
...
- AssignmentSaveController retains a created id across publish/read failures and reads list_assignments back. Success requires matching title, instructions, due time, rubric, group flag, and published status when requested. The displayed saved model comes from that canonical row. Also fixed division by zero when an assignment has no enrolled students.
- AssignmentSubmissionController reads back the submitted version and attachments before success. A partial upload keeps the same submission version and acknowledged attachment ids; retry uploads only remaining files. The form distinguishes text saved / files incomplete / confirmation unavailable and locks content after a submission receipt. In-flight saves/submissions cannot be double-clicked or dismissed through the UI.
- Regression tests were run red before the fixes: the create callback received a null deadline, a missing readback still showed success, and upload failure had no partial-success/retry state. New and strengthened widget/controller tests exercise those paths, edits, invalid dates, stale readback, missing attachments, and duplicate taps.
...
### 2026-09-23 — คำถามทุกข้อจากคลังกลายเป็นปรนัย (`d359225`)
เจ้าของงานถามว่า "หน้าคลังข้อสอบมีไว้ทำอะไร" ระหว่างตอบเลยไล่โค้ดทั้งสาย
แล้วเจอบั๊กที่ทำข้อมูลเสียโดยไม่มี error ที่จุดไหนเลย

**ต้นเหตุ 1 บรรทัด:** `teacher_question_bank_page.dart` ตัดสินชนิดคำถามด้วย
`question.type == 'essay'` แต่ enum `question_type` ใน DB มีแค่
`multiple_choice` / `true_false` / `short_answer` — **ไม่มี `'essay'`**
เงื่อนไขจึงเป็นเท็จทุกครั้ง คำถามทุกข้อที่โหลดจากหลังบ้านกลายเป็นปรนัย
(`BankQuestionType.essay` กับสีส้มของมันเป็นโค้ดตายมาตลอด)

**ลูกโซ่ 4 ด่านที่ไม่มีใครดักเลย:** ข้ออัตนัย → ดึงเข้า Exam Builder เป็นปรนัย
ที่ `options` ว่างและ `correctIndex` ถูกบังคับเป็น `0` → ด่านกันเผยแพร่เช็คแค่
`correctIndex == null` ค่าเป็น 0 จึงผ่าน → `add_quiz_question` ไม่ตรวจอะไรเลย
บันทึกสำเร็จ → **นักเรียนเจอคำถามที่ไม่มีตัวเลือกให้เลือก และเฉลยชี้ไปที่
ตัวเลือกที่ไม่มีอยู่** ระบบตรวจก็หาเฉลยไม่เจอ

แก้ทั้งสามชั้น: คลังแปลงชนิดตรง DB ครบสามแบบ + `bankTypeHasChoices`
(ถูก/ผิดก็มีตัวเลือก อย่าเทียบ `== multipleChoice`) · Exam Builder รักษาชนิด
เดิม 1:1 เลิกยัดตัวเลือกปลอม `ตัวเลือก ก/ข/ค/ง` เพิ่มชนิดถูก/ผิดในตัวเลือก
ชนิดคำถาม และด่านเผยแพร่ตรวจ "ต้องมีตัวเลือก ≥ 2" ก่อนตรวจเฉลย · RPC
`add_quiz_question` ปฏิเสธอัตนัยที่มีตัวเลือกติดมา และปรนัย/ถูกผิดที่ตัวเลือก
น้อยกว่า 2 / ข้อความตัวเลือกว่าง / เฉลยไม่ใช่หนึ่งเดียว

**เรื่องหน้าคลังข้อสอบที่เพิ่งรู้ชัด:** มันไม่ใช่หน้าเรียกดู แต่เป็น **picker
ที่ Exam Builder เปิดขึ้นมา** (`_importFromBank()` → `Navigator.push` → หน้าคลัง
`pop` `List<BankQuestion>` กลับ → ต่อท้ายข้อสอบที่กำลังร่าง) เปิดได้ 3 ทาง:
จาก Exam Builder (ทางหลัก) · เมนูหน้าแรกครู · route ตรง

**ยังไม่ได้แก้ จดไว้:**
- `create_quiz` รับ `p_lesson_id` ได้ แต่ Exam Builder **ไม่เคยส่งไป** —
  ข้อสอบทุกชุดที่ครูสร้างจากแอปไม่ผูกกับบทเรียนใดเลย (ยืนยันกับ DB จริง:
  ชุดที่สร้างจากแอปขึ้น "— ยังไม่ผูกบทเรียน —" ส่วนชุดที่ seed ผูกบทที่ 1 ปกติ)
- อาการ "กดบันทึกข้อสอบแล้วไม่สำเร็จ" ที่เจ้าของงานเจอบนมือถือ **ยังไม่รู้สาเหตุ**
  — พิสูจน์แล้วว่า RPC ทั้งสายทำงานปกติบน local สิ่งที่ขวางคือ
  `teacher_exam_builder_page.dart` catch แล้วทิ้งสาเหตุจริงลง `debugPrint`
  แล้วขึ้นข้อความรวม "บันทึกข้อสอบไม่สำเร็จ" ซึ่งบนมือถือมองไม่เห็น

**บทเรียนเรื่องวิธีทำงาน:** การแก้ชุดนี้ถูกล้างทิ้งไปรอบหนึ่งแล้ว — เซสชันหลุด
ตอนรัน test ทั้งชุด (exit 137, OOM เพราะรัน `flutter test` พร้อม `flutter analyze`
3 แพ็กเกจ) กลับมาอีกทีมี session อื่น `reset --hard` ทับ การแก้ที่ยังไม่ commit
หายหมด เหลือรอดแค่ไฟล์ untracked **commit ทันทีที่ analyze + เทสเฉพาะจุดผ่าน
อย่ารอรันเทสทั้งชุดก่อน** และอย่ารัน test กับ analyze พร้อมกันบนเครื่องนี้

**อัปเดต 2026-09-23 (บ่าย):** เจ้าของงานรัน `scripts/prod_apply_2026-09-23c.sh`
บน prod แล้ว — verify ครบ 3 ข้อ: `add_quiz_question` เหลือลายเซ็นเดียว
6 พารามิเตอร์, guard ทั้ง 3 ตัว (`choices_required` / `short_answer_takes_no_choices`
/ `exactly_one_correct_choice_required`) เป็น `true`, และ verify 3 ว่างเปล่า
— **ไม่มีข้อสอบเสียค้างอยู่บน prod เลย** ไม่ต้องตามแก้ข้อมูลเพิ่ม งานปิดครบ

### 2026-09-23 (เย็น) — รวมบัญชีซ้ำ `www.pakasit14+admin@gmail.com` เข้ากับบัญชีหลัก
เจ้าของงานถามหาวิชาคณิตศาสตร์บน prod ระหว่างตรวจก็เจอว่ามี 2 บัญชีเป็นคนเดียวกัน
สร้างเวลาเดียวกันเป๊ะ (2026-07-18 08:03:58) — `www.pakasit14@gmail.com` (6 role
ครบ แต่ชื่อยังเป็น "นักเรียน คนที่1" ค้างจากตอนสมัครแรก) กับ
`www.pakasit14+admin@gmail.com` (role เดียว school_admin ชื่อ "แอดมิน โรงเรียน")

บัญชี `+admin` ไม่ใช่บัญชีว่างเปล่า — สร้างวิชาคณิตศาสตร์ (ม.1) จริง, เชิญ+ให้สิทธิ์
`teacher@aiot-school-lab.local` เป็นครู, อนุมัติ parent_links, ออกรหัส parent
binding, ลงทะเบียนนักเรียน 1 คน, มี audit_logs 34 แถว ก่อนลบต้องย้ายทุกอย่างไป
บัญชีหลักก่อน ไม่ใช่ลบทิ้งเฉย ๆ — รันผ่าน `scripts/prod_apply_2026-09-23d.sh`
(เจ้าของงานรันเอง, verify ครบ 3 ข้อ): เปลี่ยนชื่อบัญชีหลักเป็น "ทดสอบ ทุกสิทธิ์"
ย้าย courses.created_by / users.created_by / user_roles.granted_by /
user_invitations.invited_by / parent_binding_codes.issued_by /
parent_links.(first_reviewed_by, approved_by) / course_students.enrolled_by /
audit_logs.user_id ไปบัญชีหลักทั้งหมด ลบ session/OTP/trusted-device (แค่ประวัติ
ล็อกอิน ไม่มีความหมายให้เก็บ) แล้วลบบัญชี `+admin` role `school_admin` ที่ซ้ำกับ
บัญชีหลักก็ถูกลบไปพร้อมกัน — บัญชีหลักยังมีครบ 6 role, วิชาคณิตศาสตร์ผูกกับบัญชี
หลักถูกต้องหลังรวม

**ด่านตรวจที่เพิ่มในสคริปต์ 23d (`2a025a6`, แก้ escape `5913723`):** ตอนรีวิว
สคริปต์พบว่ามี FK ชี้มาที่ `users.id` ทั้งหมด **136 คอลัมน์** แต่สคริปต์ย้าย
ไว้ 12 — ที่เหลือส่วนใหญ่เป็น `NO ACTION` ซึ่งปลอดภัย (ลบแล้ว error ดัง ๆ)
แต่ **15 คอลัมน์เป็น `CASCADE`/`SET NULL`** (`teacher_subjects`,
`staff_profiles`, `meeting_attendees`, `device_schedules`,
`control_approval_requests`, `staff_requests` ฯลฯ) ถ้าบัญชีซ้ำมีแถวใน
ตารางพวกนั้นแม้แถวเดียว การลบจะลากข้อมูลหายไปด้วยแบบไม่มีคำเตือน จึงเพิ่ม
ด่าน 2 จุด (ขั้น 0 ก่อนแตะอะไร และขั้น 2.5 หลังย้ายประวัติก่อนลบจริง) ที่
ไล่ FK ทุกคอลัมน์แบบ dynamic จาก `information_schema` — ไม่ใช่รายชื่อตาราง
ที่พิมพ์ไว้ตายตัว ตารางใหม่ที่เพิ่มมาทีหลังจึงถูกตรวจเองอัตโนมัติ

**กับดักที่เสียเวลาไปหนึ่งรอบ:** SQL ที่แทรกเพิ่มเขียน `do $$` กับ
`where %I = $1` ไว้ในสตริง double quote ของ bash ตรง ๆ — `$$` กลายเป็น PID
และ `$1` เป็นพารามิเตอร์ตัวแรกของสคริปต์ซึ่งไม่มี `set -u` เลยหยุดที่
`unbound variable` ตั้งแต่ด่านแรกตอนเจ้าของงานรันจริง ของเดิมในไฟล์ escape
เป็น `\$\$` ไว้ถูกแล้ว **สคริปต์ในโฟลเดอร์นี้ทุกตัวที่ฝัง SQL ต้อง escape
`$` เสมอ** ตรวจได้โดยดัก SQL ที่จะยิงจริงผ่าน `npx` ตัวปลอมแล้วเอาไปรันกับ
local ก่อน อย่าเชื่อแค่ `bash -n` ซึ่งไม่จับเรื่องนี้

**สถานะบัญชีหลังรวม (ยืนยัน 2026-09-23 เย็น):** `select ... like 'www.pakasit14%'`
บน prod เหลือแถวเดียว `www.pakasit14@gmail.com` / "ทดสอบ ทุกสิทธิ์" — และ
บัญชีนี้เป็นครูผู้สอนวิชาคณิตศาสตร์บน prod จริง (พบตอนรัน
`scripts/prod_seed_sample_quiz.sh`) จึงใช้ล็อกอินทดสอบแอปบนมือถือได้

### 2026-09-23 — ผูกข้อสอบกับบทเรียนได้แล้ว (`f3eefb6`) + แสดงสาเหตุตอนบันทึกไม่ผ่าน (`7f7a9de`)

**`p_lesson_id` ที่ไม่เคยมีใครส่ง** — `create_quiz` รับพารามิเตอร์นี้มาตั้งแต่
migration แรก (20260818000000) แต่ `teacher_exam_builder_page.dart` ส่งแค่
courseId/type/title/timeLimitMin ข้อสอบทุกชุดที่ครูสร้างจากแอปจึงมี
`lesson_id = null` หน้าไหนที่จัดของตามบทเรียนจะมองไม่เห็นข้อสอบพวกนี้เลย
เพิ่มปุ่มเลือกบทเรียนในแถบตั้งค่า โหลดจาก `LessonService.listLessons`
**เจตนา:** เลือก "ยังไม่ผูกบทเรียน" ได้ (ข้อสอบรวมปลายภาคไม่สังกัดบท) ·
วิชาที่ยังไม่มีบทเรียนยังสร้างข้อสอบได้ · โหลดบทเรียนพังก็ไม่กันการบันทึก
**ยังไม่มีด่าน:** `create_quiz` ไม่ตรวจว่า `lesson_id` เป็นบทของวิชานั้นจริง
ตอนนี้กันด้วย UI อย่างเดียว

**ข้อความตอนบันทึกไม่ผ่าน** — เดิมทุก error ขึ้นว่า "บันทึกข้อสอบไม่สำเร็จ"
เหมือนกันหมด สาเหตุจริงถูกทิ้งลง `debugPrint` ซึ่งบนเครื่องจริงไม่มีใครเห็น
เจ้าของงานเจออาการนี้บนไอโฟนแล้วไล่ต่อไม่ได้เลย เพิ่ม `examSaveErrorMessage`
แปลโค้ดที่ RPC โยนมาเป็นข้อความที่บอกว่าต้องทำอะไรต่อ ครอบคลุม
invalid_session/forbidden (บอกด้วยว่าต้องเข้าสู่ระบบด้วยบทบาทครู — บัญชี
หลายบทบาทเป็นกับดักจริงของโปรเจกต์นี้) / ด่านใหม่ทั้ง 5 ตัวจาก 20260923020000
/ เน็ตหลุด **error ที่ยังไม่มีใครแปลจะโชว์ของจริงออกมา (ตัดที่ 160 ตัวอักษร)
ไม่กลืนทิ้ง** — เจอครั้งหน้าจะได้รู้ว่าต้องเพิ่มอะไร

**ยังไม่ได้ทดสอบบนเครื่องจริง** — build release ผ่าน (311 วินาที, framework
เซ็นครบ) แต่ติดตั้งไม่ได้เพราะไอโฟนไม่ได้ต่ออยู่ (`devicectl` ขึ้น
`unavailable`, `system_profiler` ไม่เห็น iPhone บน USB เลย = ปัญหาสายหรือ
การเชื่อมต่อ ไม่ใช่ pair/trust) ไฟล์ `build/ios/iphoneos/Runner.app` พร้อม
ติดตั้งอยู่แล้ว ต่อเครื่องแล้วรัน `devicectl device install` ได้เลย ไม่ต้อง
build ใหม่ · อาการ "กดบันทึกแล้วไม่สำเร็จ" ที่เป็นคำถามตั้งต้นจึงยังไม่รู้
สาเหตุ แต่แอปเวอร์ชันถัดไปจะบอกเองแล้ว
