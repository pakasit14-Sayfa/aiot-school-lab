...
**อัปเดต — §4 นักเรียน/ครู เสร็จแล้ว §4.4 แอดมินยังไม่ได้ทำ**: รายงานเต็มที่
[`ANDROID_ROUND1_REPORT_2026-09-21.md`](ANDROID_ROUND1_REPORT_2026-09-21.md)
สร้าง AVD 3 ตัวตรง §3 (API 34 ทั้งหมด หลัง downgrade จาก API 37 ที่มีอยู่)
**พบบั๊กจริง 2 จุด**: (1) ชื่อชุดข้อมูลเซนเซอร์ตัวแรกในใบงานนักเรียนเพี้ยนเป็น
ตัวอักษรมั่ว (2) จำนวนนักเรียนในวิชาของครูไม่ตรงกันระหว่างอุปกรณ์ (tablet
3 คน, มือถือ 0 คน ซ้ำ 2 ครั้ง) **พบพฤติกรรม back-button ที่ใบสั่งงานกังวลไว้
ล่วงหน้าจริง**: กด back ครั้งเดียวจากบางบริบท (drawer/หน้า push) แล้วแอปปิด
ออกไป Android home เลยแทนที่จะ pop ทีละหน้า — ไม่ reproduce สม่ำเสมอ (~4/6
ครั้ง) ต้องหาคนเขียน `PopScope`/`WillPopScope` ที่ขาดในจุดที่เจอ **ตารางสอน
ครูทดสอบไม่จบ** เพราะเครื่อง dev รัน 3 emulator พร้อมกันจน RAM เหลือ
~185MB แอปถูก Android trim กลางคัน — สงสัยว่าไม่ใช่บั๊กจริง แต่ยังไม่ได้
ยืนยันซ้ำด้วยเครื่องที่ไม่ตึง resource

**อัปเดต 2 — ลอง §4.4 แอดมินต่อ (emulator เดียว ไม่ตึง RAM แล้ว) เจอปัญหา
ใหญ่กว่าที่คิด**: ล็อกอินแอดมิน หน้าหลักโหลดผ่านปกติ (3 นักเรียน/2 ครู/5
อุปกรณ์) แต่เมนู "จัดตารางเรียน" โหลดไม่สำเร็จ (error state สุจริต) ลอง
force-restart แอปทั้งตัวแล้วเข้าใหม่ — ค้าง loading >1 นาทีแล้ว **แอป
auto-navigate กลับหน้าหลักเอง** และ **หน้าหลักเองก็เริ่ม fail ตามไปด้วย**
("โหลดข้อมูลสรุปไม่สำเร็จ") ทั้งที่เพิ่งโหลดผ่านมาก่อนหน้านี้ในเซสชัน
เดียวกัน — **สรุปว่าไม่ใช่บั๊กเฉพาะหน้า "จัดตารางเรียน"** แต่เป็นอาการ
session/connection เสื่อมจากการทดสอบต่อเนื่องยาว (~2 ชม.+ ผ่าน RAM crisis
+ restart แอปหลายรอบมาก่อน) ยังไม่ได้ไล่ทดสอบส่วนที่เหลือของ §4.4 (ภาพรวม
→ ห้อง → ปฏิทินสัปดาห์) เพราะเข้าเมนูไม่ได้เลยในสภาพนั้น **ต้องทดสอบซ้ำ
ด้วยเซสชันสั้นสดใหม่ก่อนสรุปว่าเป็นบั๊กแอปจริงหรือแค่สภาพแวดล้อม dev**
