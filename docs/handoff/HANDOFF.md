...
**Learning tracks (2026-09-05):** controller/page/service rewired the same way (`school_admin_learning_tracks_controller.dart`, canonical `{tracks, rooms}` snapshot, per-action busy keys, no fake success). Two real bugs found and fixed while connecting it: the room-assignment dropdown used `DropdownButtonFormField`'s `initialValue:` param, which doesn't exist on this project's pinned Flutter SDK (`^3.8.1`) — the exact bug class already documented above for CCTV/device-schedule, fixed back to `value:`; and a new pgTAP file found all 7 Learning-track RPCs had an unintended `service_role` EXECUTE grant, fixed with append-only `20260905000000_learning_tracks_service_role_acl.sql` (no signature change). Verification: analyzer clean, focused Flutter 26/26 (12 controller + 14 connection), full 15-file cumulative suite 93 passed/8 failed (101 total, same known screenshot-path cases), 14-file non-screenshot suite 101/101, new `34_school_admin_learning_tracks.test.sql` pgTAP 58/58, full local DB suite has only pre-existing unrelated failures. REST named-parameter smoke test covered every RPC plus one wrong-role and one cross-school rejection, all temporary rows cleaned up afterward. Live browser click-through was not performed this pass. Progress is 5/20 pages (25%). Next page is Permissions, followed by the entire prior regression ledger again.
