...
Alerts is committed as `c8346ff`, CCTV as `830e20a`, and Device schedules as `b27f843`. Incident inbox is implemented and ready for commit. All four pages use small injectable controllers over RPC-backed services. Incident acknowledge/close refetch canonical data before reporting success, use a per-item mutation lock, preserve the close note/dialog on failure, and load details through a new school-scoped staff RPC. Empty cards use `ยังไม่มีข้อมูล`.

Verification (re-run after fixing a real SQL syntax error and a real ambiguous-finder test bug found picking this up — see `task_plan.md`'s 2026-09-04 correction entry for detail): scoped analyzer clean, Incident inbox tests 15/15, cumulative School Admin regression 67/75, and legacy + new incident pgTAP 51/51. The new append-only migrations add the staff detail RPC, restore close action/audit records for regular and escalated incidents, revoke `PUBLIC` execution from inbox RPCs, and revoke unnecessary `service_role` execution (applied and recorded in `schema_migrations` — it existed as a file but had never been run); `super_admin` scope remains unchanged by explicit decision. Progress is 4/20 pages (20%). Eight screenshot tests still reference a stale absolute `/Users/sayfa/...` artifact path and remain outside this slice — same known count as before, not new failures. Next page is Learning tracks, followed by the entire prior regression ledger again.
