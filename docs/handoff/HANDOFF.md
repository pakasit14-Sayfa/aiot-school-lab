...
> รายละเอียดและข้อจำกัด DB จริงดู [บรีฟบั๊ก 1 ฉบับอัปเดต](FIX_BRIEF_FOR_CODEX.md)
...
Verification (re-run after fixing a real SQL syntax error and a real ambiguous-finder test bug found picking this up — see `task_plan.md`'s 2026-09-04 correction entry for detail): scoped analyzer clean, Incident inbox tests 15/15, full 13-file cumulative suite: 75 passed, 8 failed (83 total); all 8 failures are the pre-existing stale `/Users/sayfa/...` screenshot-path cases. The separate 12-file non-screenshot suite passes 75/75. Incident pgTAP passes 50/50 targeted and 63/63 combined. The new append-only migrations add the staff detail RPC, restore close action/audit records for regular and escalated incidents, revoke `PUBLIC` execution from inbox RPCs, and revoke unnecessary `service_role` execution (applied and recorded in `schema_migrations` — it existed as a file but had never been run); `super_admin` scope remains unchanged by explicit decision. Eight screenshot tests still reference a stale absolute `/Users/sayfa/...` artifact path and remain outside this slice — same known count as before, not new failures. A local named-parameter REST smoke call to `get_incident_report_for_staff` used the exact JSON keys `p_token` and `p_id`: same-school staff received one canonical row (HTTP 200), while a wrong-role session failed closed with `forbidden` (HTTP 400). `DATABASE_SCHEMA.md` was regenerated from the live local catalog and its exact function signature/return columns match `pg_proc`.

**Learning tracks (2026-09-05):** controller/page/service rewired the same way (`school_admin_learning_tracks_controller.dart`, canonical `{tracks, rooms}` snapshot, per-action busy keys, no fake success). Three real bugs found and fixed while connecting it: (1) the room-assignment dropdown used `DropdownButtonFormField`'s `initialValue:` param, which doesn't exist on this project's pinned Flutter SDK (`^3.8.1`) — the exact bug class already documented above for CCTV/device-schedule, fixed back to `value:`; (2) a new pgTAP file found all 7 Learning-track RPCs had an unintended `service_role` EXECUTE grant, fixed with append-only `20260905000000_learning_tracks_service_role_acl.sql` (no signature change); (3) the track dialog's `nameController.dispose()` ran immediately after `showDialog` returned while a rebuild could still touch that field, throwing "TextEditingController used after being disposed" — confirmed by isolation (removing only the `dispose()` call fixed it even without any other change) that this, not a Flutter framework defect, was the cause; fixed the same way the incident-inbox close dialog already handles its own `noteController` (not disposed). Verification: analyzer clean, focused Flutter 26/26 (12 controller + 14 connection), full 15-file cumulative suite 109 tests — 101 passed, 8 failed (same known screenshot-path cases; re-verified against the raw `+101 -8` runner counters after an earlier draft of this note miscounted the total as 101), 14-file non-screenshot suite 101/101, new `34_school_admin_learning_tracks.test.sql` pgTAP 58/58, full local DB suite has only pre-existing unrelated failures. REST named-parameter smoke test covered every RPC plus one wrong-role and one cross-school rejection, all temporary rows cleaned up afterward. **Live browser click-through was not performed this pass** — code and automated verification are complete, but this page is not yet confirmed end-to-end in a real browser; don't call it production-ready until that's done. Progress is 5/20 pages (25%). Next page is Permissions, followed by the entire prior regression ledger again.
