...
### 6. `school_admin_device_control_page` (783)

ต่อจริง: `listSchoolDevices` `queueDeviceCommand`

| ปัญหา | verdict | ทางแก้ |
|---|---|---|
| ขึ้น "ส่งคำสั่ง เปิด X สำเร็จ" ทั้งที่แค่ **เข้าคิว** ยังไม่รู้ว่าอุปกรณ์ทำจริงไหม | **A** | `ack_device_command` `poll_device_commands` `list_device_relay_states` มีครบ → แยกสถานะ `queued` / `acknowledged` / `applied` ให้เห็นจริง |
| UI เริ่มที่ OFF เสมอ ไม่ได้อ่านสถานะจริง | **A** | `list_device_relay_states` |

**นี่คือหน้าที่ "โกหก" ชัดที่สุดในกลุ่ม** — ผู้ดูแลกดปิดไฟแล้วเชื่อว่าไฟดับ
...
| 2 | `device_control` | A | หน้าที่โกหกผู้ใช้ชัดที่สุด (บอกสำเร็จตอนแค่เข้าคิว) แก้ด้วย RPC ที่มีอยู่แล้ว | 1 |
