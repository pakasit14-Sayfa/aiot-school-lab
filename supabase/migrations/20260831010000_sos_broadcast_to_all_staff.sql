-- =====================================================================
-- SOS reports were only reaching teachers who share a course/room with
-- the reporting student. Testing found a school with zero courses set
-- up yet (no course_teachers/course_students rows at all) meant an SOS
-- report reached literally nobody — the RPC still returned success, so
-- the reporting student had no way to know nobody was notified.
--
-- Fix, per explicit decision: for category = 'sos' specifically (life
-- -safety, needs broad awareness), notify/allow EVERY teacher,
-- school_admin, and executive at the school — not just a narrowly
-- matched subset. Non-SOS ("แจ้งเหตุผิดปกติ") reports keep the original
-- narrow course/room-matched routing, since blasting all staff for
-- every minor report would be noisy. `executive` is also newly added to
-- the allowed-actor role list throughout (previously excluded
-- entirely, even from viewing/acknowledging any incident).
-- =====================================================================
...
begin
  ...
  if p_category = 'sos' then
    -- SOS: กระจายให้ครู/แอดมินโรงเรียน/ผู้บริหารทุกคนในโรงเรียน ไม่จำกัด
    -- เฉพาะครูที่สอน/เป็นครูที่ปรึกษาของนักเรียนคนนี้โดยตรง
    for v_teacher in
    ...
    -- แจ้งเตือนครูที่:
    -- 1) สอนนักเรียนผู้แจ้งเหตุโดยตรง
    -- 2) สอนในห้องเรียนที่เกิดเหตุ
    -- 3) สอนนักเรียนในห้องประจำชั้น (Homeroom) เดียวกัน
    for v_teacher in
      ...
      where c.school_id = v_actor.school_id
        and (
          cst.student_id = v_actor.user_id
          or (v_room is not null and c.room = v_room)
          or (v_room is not null and sp.room = v_room)
        )
    loop
      insert into notifications (user_id, type, title, body, payload)
      values (
...
begin
  ...
  where ir.school_id = v_actor.school_id
    ...
    and (
      v_actor.role in ('school_admin', 'executive')
      or ir.category = 'sos'
      or exists (
        select 1
        from course_teachers ct
