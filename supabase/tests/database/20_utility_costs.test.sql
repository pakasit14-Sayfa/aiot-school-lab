...
-- Test 1-4: get_school_utility_rates returns defaults initially
select results_eq(
...
-- Test 5: set_school_utility_rates fails for student role (forbidden)
select throws_ok(
...
-- Test 6: set_school_utility_rates fails for non-positive rate
select throws_ok(
...
-- Test 7: set_school_utility_rates succeeds for school_admin
select lives_ok(
  $$ select set_school_utility_rates('utl-admin-token', 5.20, 22.50) $$,
  'school_admin can set utility rates'
);

-- Test 8: get_school_utility_rates returns updated custom rates
select results_eq(
  $$ select electricity_rate_thb, is_electricity_default, water_rate_thb, is_water_default from get_school_utility_rates('utl-admin-token') $$,
...
-- Test 9: get_energy_usage_summary returns row structure & disclaimer
select isnt_empty(
...
-- Test 11: get_water_usage_summary returns row structure & disclaimer
select isnt_empty(
...
-- Test 13-14: Seed dummy meter device and reading to verify calculation math
do $$
declare
  v_dev_elec uuid;
  v_dev_water uuid;
