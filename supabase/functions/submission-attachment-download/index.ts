...
// `submission-attachments` bucket. get_submission_attachment_for_download
// re-checks ownership/course-teacher membership on every call. Mirrors
...
Deno.serve(async (req) => {
  ...
  if (!serviceRoleKey) {
    console.error("submission-attachment-download server configuration unavailable");
    return json({ error: "invalid_request" }, 400);
  }
  ...
  const lookup = await supabase.rpc("get_submission_attachment_for_download", {
    p_token: token,
