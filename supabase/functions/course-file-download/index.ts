...
// `course-files` bucket. get_course_file_for_download re-checks course
// membership (teacher/enrolled student/admin) on every call.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    fileId = typeof body.file_id === "string" ? body.file_id : null;
  } catch {
  ...
  if (!serviceRoleKey) {
    console.error("course-file-download server configuration unavailable");
    return json({ error: "invalid_request" }, 400);
  }
  ...
  const lookup = await supabase.rpc("get_course_file_for_download", {
    p_token: token,
    p_file_id: fileId,
  }).maybeSingle();
