...
// `leave_attachments` bucket. assert_leave_attachment_upload_access
// (parent role only) gates every path, matching this project's
// RPC-only/deny-all philosophy. Mirrors submission-attachment-upload.
...
Deno.serve(async (req) => {
  ...
  if (!serviceRoleKey) {
    console.error("leave-attachment-upload server configuration unavailable");
    return json({ error: "invalid_request" }, 400);
  }
  ...
  const access = await supabase.rpc("assert_leave_attachment_upload_access", {
    p_token: token,
  });
  if (access.error) {
