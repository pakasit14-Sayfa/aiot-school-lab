...
// `staff-leave-attachments` bucket. get_staff_leave_attachment_for_download
// re-checks on every call who may read the file — the requester, or the
// school's admin/executive. Mirrors school-report-download.
...
Deno.serve(async (req) => {
  ...
  if (!serviceRoleKey) {
    console.error("staff-leave-attachment-download server configuration unavailable");
    return json({ error: "invalid_request" }, 400);
  }
  ...
  const lookup = await supabase.rpc(
    ...
    { p_token: token, p_attachment_id: attachmentId },
  ).maybeSingle();
