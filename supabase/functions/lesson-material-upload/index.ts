...
// `lesson-materials` bucket. Clients never touch storage.objects
// directly — assert_lesson_upload_access (teacher of the lesson's
// course) gates every path, matching this project's RPC-only/deny-all
// philosophy.
...
Deno.serve(async (req) => {
  ...
  if (!token || !lessonId || !fileName) {
    return json({ error: "invalid_request" }, 400);
  }

  const serviceRoleKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
  if (!serviceRoleKey) {
    console.error("lesson-material-upload server configuration unavailable");
    return json({ error: "invalid_request" }, 400);
  }
  ...
  const access = await supabase.rpc("assert_lesson_upload_access", {
    p_token: token,
  ...
  const storagePath = `${lessonId}/${crypto.randomUUID()}-${
