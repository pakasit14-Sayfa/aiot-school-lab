...
| 6 | Learning tracks | [x] | [x] | [x] | [x] | [x] | analyzer clean; focused 26/26 (12 controller + 14 connection); full 15-file cumulative suite: 93 passed, 8 failed (101 total), all 8 the pre-existing stale `/Users/sayfa/...` screenshot-path cases; the 14-file non-screenshot suite passes 101/101. Targeted pgTAP `34_school_admin_learning_tracks.test.sql` 58/58; full local DB suite 33 files, pre-existing unrelated failures only (auth rate-limit, tenant isolation, login 2FA, two facility_manager-role files that no longer parse post role-merge, emergency events — none reference Learning tracks). REST named-parameter smoke (list/create/update/list-rooms/set-room/clear/delete) all confirmed via canonical refetch; one wrong-role (teacher) and one cross-school request both failed closed (`forbidden`/`track_not_found`); all temporary rows/sessions cleaned up. One append-only migration (`20260905000000_learning_tracks_service_role_acl.sql`) revokes an unintended `service_role` EXECUTE grant found live on all 7 RPCs (same shape as the incident-inbox ACL fix) — no signature/behavior change, `DATABASE_SCHEMA.md` left as-is since no schema/signature changed. |
| 7 | Permissions | [ ] | [ ] | [ ] | [ ] | [ ] | |
