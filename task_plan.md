...
- ~~Latest `list_school_admin_audit_logs` includes rows where `school_id IS NULL`; a school admin must not receive global/null-school audit rows. Add an append-only hardening migration and isolation test.~~ **Fixed 2026-09-05** (`20260905040000_harden_school_admin_audit_scope.sql`, test `36_school_admin_audit_scope.test.sql`, 9/9 pass) — narrowed the school_admin branch to an exact `school_id` match; super_admin's cross-school scope is unchanged. Also found and fixed the same `service_role` EXECUTE leak already documented for the incident-inbox/learning-track RPCs (this function's grants had never revoked `service_role`, only `PUBLIC`/`anon`, since it was first created).
...
| 5 | Incident inbox | [x] | [x] | [x] | [x] | [x] | analyzer clean; focused 15/15; full 13-file cumulative suite: 75 passed, 8 failed (83 total); all 8 failures are the pre-existing stale `/Users/sayfa/...` screenshot-path cases. The separate 12-file non-screenshot suite passes 75/75. Incident pgTAP passes 50/50 targeted and 63/63 combined; named-parameter REST detail smoke (`p_token`, `p_id`) returned one canonical row for same-school staff and `forbidden` for a wrong-role caller. |
| 6 | Learning tracks | [x] | [x] | [x] | [x] | [x] | analyzer clean; focused 26/26 (12 controller + 14 connection); full 15-file cumulative suite: 109 tests, 101 passed, 8 failed, all 8 the pre-existing stale `/Users/sayfa/...` screenshot-path cases (unchanged failure set, re-verified against the raw runner output — an earlier draft of this row miscounted this as "93 passed, 8 failed (101 total)"); the 14-file non-screenshot suite passes 101/101. Targeted pgTAP `34_school_admin_learning_tracks.test.sql` 58/58; full local DB suite 33 files, pre-existing unrelated failures only (auth rate-limit, tenant isolation, login 2FA, two facility_manager-role files that no longer parse post role-merge, emergency events — none reference Learning tracks). REST named-parameter smoke (list/create/update/list-rooms/set-room/clear/delete) all confirmed via canonical refetch; one wrong-role (teacher) and one cross-school request both failed closed (`forbidden`/`track_not_found`); all temporary rows/sessions cleaned up. One append-only migration (`20260905000000_learning_tracks_service_role_acl.sql`) revokes an unintended `service_role` EXECUTE grant found live on all 7 RPCs (same shape as the incident-inbox ACL fix) — no signature/behavior change, `DATABASE_SCHEMA.md` left as-is since no schema/signature changed. **Live browser click-through not done** — code and automated verification (analyzer, unit/widget tests, pgTAP, REST) are complete, but this page has not been clicked through in a real running browser; do not treat it as fully end-to-end verified until that's done. |
| 7 | Permissions | [ ] | [ ] | [ ] | [ ] | [ ] | |
...
- **Correction (2026-09-04, same day): the "14/14, 76/76, 36/36" numbers above were recorded before the newest pgTAP/migration/test-fix pass and were never actually reproducible.** Picking this up from a fresh handoff found two real, concrete problems: (1) `33_school_admin_incident_inbox.test.sql` had a genuine SQL syntax error — two `select is(...)` statements had 4 unrelated `throws_ok(...)` blocks physically interleaved inside them (a bad merge), so the file died at `psql` parse time after only 14 of the planned 38 assertions ran; (2) `school_admin_incident_inbox_connection_test.dart`'s "failed close keeps dialog and note" test used `find.textContaining('ปิดเหตุการณ์ไม่สำเร็จ')`, which is genuinely ambiguous — the dialog's own retry SnackBar and the controller's background error state both legitimately contain that substring at the same time, so the finder matched 2 widgets instead of 1. Both fixed for real: the SQL statements were reassembled into valid syntax (no assertions removed, all 4 interleaved `throws_ok` blocks preserved), the missing 38th assertion (service_role EXECUTE revoked, mirroring the existing PUBLIC check) was added per the handoff spec, migration `20260904010200_incident_inbox_service_role_acl.sql` was applied locally and recorded in `schema_migrations` (it existed as a file but had never been run), and the Dart test now asserts the exact dialog SnackBar text instead of a substring. **Actual final counts, all independently re-run after the fixes**: targeted Flutter (controller + connection) 15/15, full 13-file cumulative School Admin suite: 75 passed, 8 failed (83 total); all 8 failures are the pre-existing stale `/Users/sayfa/...` screenshot-path cases. The separate 12-file non-screenshot suite passes 75/75. Combined incident pgTAP (`19_incident_reports.test.sql` + `33_school_admin_incident_inbox.test.sql`) passes 63/63, targeted Incident pgTAP passes 50/50, and targeted `flutter analyze` is clean. A local named-parameter REST call to `get_incident_report_for_staff` with JSON keys `p_token` and `p_id` returned one canonical same-school row (HTTP 200); the same shape with a wrong-role session failed closed with `forbidden` (HTTP 400). `docs/handoff/DATABASE_SCHEMA.md` was regenerated from the live local database; the exact `get_incident_report_for_staff(text, uuid)` signature and return columns were independently matched against `pg_proc`. Progress: 4/20 pages (20%). Next implementation target: Learning tracks.

- **Learning tracks (2026-09-05): connected with a controller seam matching the four prior pages.** New `SchoolAdminLearningTracksController` (`school_admin_learning_tracks_controller.dart`) publishes one canonical `{tracks, rooms}` snapshot loaded concurrently, uses per-action busy keys (`create`, `track:<id>`, `room:<gradeLevel>/<room>`) so one item's mutation never blocks another, and only reports success after a canonical refetch confirms the exact expected fields (id+name+color for create, +sortOrder for update, absence for delete, exact `(gradeLevel, room, trackId)` for assign/clear). `LearningTrackService` now fails locally with `not_signed_in` before any RPC call and validates `create_learning_track`'s returned id is non-empty (matching the codebase-wide `Exception('...')` convention, not a bespoke `StateError`). The page was rewired onto the controller, both empty cards now show the exact heading `ยังไม่มีข้อมูล`, mutation failures show stable Thai copy (never `$e`/raw backend text) and keep the last confirmed snapshot, and the track dialog's busy state is driven by `AnimatedBuilder(animation: _controller)` (matching the incident-inbox close-dialog pattern) rather than a disconnected local flag — a deliberate architecture choice, not a bug fix in itself. **A genuine, isolated app-level bug was found and fixed while writing the widget tests**: the dialog's `nameController.dispose()` ran immediately after `showDialog` returned, but a rebuild (from the mutation's own settling, or the dialog's pop transition) could still touch that `TextField` afterward, throwing "A TextEditingController was used after being disposed" and, upstream of that, a `pumpAndSettle` timeout / `AnimatedDefaultTextStyle` build-scope assertion. Verified by isolation: reverting the dialog to its original local-`StatefulBuilder`-only design (no `AnimatedBuilder`) but removing only the `dispose()` call also fixed the crash — confirming the disposal timing, not a Flutter framework defect, was the actual cause. Fixed the same way the incident-inbox close dialog already handles its own `noteController` (not disposed, for the same reason). Also fixed a real copy-paste regression from an earlier draft: the room dropdown used `DropdownButtonFormField`'s newer `initialValue:` param, which doesn't exist on this project's pinned Flutter SDK (`^3.8.1` in `pubspec.yaml`, CI pinned to `3.8.1`) — same class of bug HANDOFF.md already documents being fixed once for CCTV/device-schedule; changed back to `value:` to match every other dropdown in `school_admin/`.
  **Verification**: targeted `flutter analyze` clean on all 4 changed Dart files (only the pre-existing `value:`-deprecation info-level note, reproduced identically on the already-shipped CCTV/device-schedule pages under this workstation's newer local Flutter SDK — not present under the pinned `3.8.1`). Focused Flutter 26/26 (12 controller + 14 connection, including a real loading-spinner assertion, real create/update/delete/assign/clear success paths, and a per-control busy/disabled assertion — the original draft only covered failure paths for those). Full 15-file cumulative School Admin suite: 109 tests, 101 passed, 8 failed, all 8 the same pre-existing stale `/Users/sayfa/...` screenshot-path cases (not new, re-verified against the raw `flutter test` runner output — `+101 -8` cumulative counters, i.e. 101 passed and 8 failed, 109 total, not "101 total" as an earlier draft of this entry miscounted); the 14-file non-screenshot suite passes 101/101. New `supabase/tests/database/34_school_admin_learning_tracks.test.sql` passes 58/58, covering RLS/deny-all, no `PUBLIC` execute, missing/invalid token fail-closed for every page-called RPC, wrong-role (teacher) and read-only-widening (executive/super_admin blocked from every mutation and from the school_admin-only room list) rejections, null-active-school sessions revealing no data and failing every mutation closed, duplicate-name/cross-school isolation, and cascade-on-delete. Writing this pgTAP file caught a real, previously-unknown defect: all 7 Learning-track RPCs had `EXECUTE` granted to `service_role` despite the migration only ever granting `anon, authenticated` — same shape as the incident-inbox `service_role` leak fixed in `20260904010200`. Fixed with the append-only `20260905000000_learning_tracks_service_role_acl.sql` (revokes only; no signature change, so `DATABASE_SCHEMA.md` was left as-is). Full local `npx supabase test db` (33 files) shows only pre-existing, unrelated failures (`03_auth_session_rate_limit`, `05_tenant_isolation`, `07_login_2fa`, `14_facility_manager_building_scope`/`16_facility_manager_device_list` — both now parse-fail post role-merge, `22_emergency_events`) — none reference `learning_track`, and a static read of the new migration confirms it cannot affect any of them. REST named-parameter smoke test against local Supabase: list → create → canonical list → update → canonical list → room list → assign → canonical room list → clear → canonical room list → delete → canonical list, one wrong-role request (teacher calling `list_learning_tracks`/`create_learning_track`, both `403 forbidden`) and one cross-school request (a throwaway second school's admin updating school A's track, `400 track_not_found`, row confirmed unchanged after). All temporary sessions/users/school/package created for the REST smoke test were deleted afterward. **Code and automated verification (analyzer, unit/widget tests, pgTAP, REST) are complete; live browser click-through of this page has not been done.** REST-level verification exercises the same session/RPC path the UI calls, but it is not a substitute for actually clicking through the real page — do not treat Learning tracks as fully end-to-end verified, and do not report it as production-ready, until that click-through happens. Progress: 5/20 pages (25%). Next implementation target: Permissions.
