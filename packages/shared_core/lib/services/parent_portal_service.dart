...
class ParentPortalService {
  ...
  static Future<List<StudentAttendanceItem>> listMyStudentAttendance(
    ...
    return rows
        .map(
          (row) => StudentAttendanceItem.fromRow(row as Map<String, dynamic>),
        )
        .toList();
  }
  ...
  static Future<List<StudentAssignmentItem>> listMyStudentAssignmentItems(
    String studentId,
  ) async {
    final rows = await listMyStudentAssignments(studentId);
  ...
  static Future<void> submitLeaveRequest({
    ...
    if (attachmentFile != null) {
      final fileName =
          '${DateTime.now().millisecondsSinceEpoch}_$studentId.jpg';
