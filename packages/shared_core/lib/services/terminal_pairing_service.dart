...
class TerminalPairingService {
  static Future<({String pairingCode, DateTime expiresAt})>
  createPairingSession({String? terminalName}) async {
    final rows =
        await supabase.rpc(
  ...
  static Future<TerminalPairingPeek> peekPairingSession(
    String pairingCode,
  ) async {
    final rows =
        await supabase.rpc(
  ...
  static Future<({bool success, String studentName, String message})>
  claimPairingSession(String pairingCode) async {
    final rows =
        await supabase.rpc(
  ...
  static Future<TerminalPairingResult> checkPairingStatus(
    String pairingCode,
  ) async {
    final rows =
        await supabase.rpc(
