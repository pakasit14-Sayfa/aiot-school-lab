...
class _IssueBindingCodePageState extends State<IssueBindingCodePage> {
  ...
  Future<void> loadCodes() async {
    ...
    try {
      final result = await ParentBindingService.listBindingCodes();
  ...
  void issueCode() async {
    ...
    try {
      final result = await ParentBindingService.createBindingCode(
  ...
  void revokeCode(BindingCode code) async {
    try {
      await ParentBindingService.revokeBindingCode(code.id);
      await loadCodes();
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
