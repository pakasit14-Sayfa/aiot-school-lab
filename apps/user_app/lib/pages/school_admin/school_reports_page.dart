...
class _SchoolReportsPageState extends State<SchoolReportsPage> {
  ...
  void _createReport(String format) {
    _message('ระบบสร้างรายงานรูปแบบ $format ยังอยู่ระหว่างการพัฒนาและยังไม่เชื่อมต่อระบบหลังบ้าน');
  }
  ...
  Widget _summary() {
    ...
    final data = [
      _Summary('นักเรียน', s != null ? s.studentsCount.toString() : '--', 'รายชื่อในระบบโรงเรียน', Icons.groups_rounded,
          SchoolAdminPalette.primaryDark),
      _Summary('อุปกรณ์', s != null ? '${s.devicesCount}' : '--', s != null ? 'ออนไลน์ ${s.devicesOnline}' : 'ลงทะเบียนในระบบ',
          Icons.memory_rounded, const Color(0xFF4F6078)),
      _Summary('อาคาร / ห้อง', s != null ? '${s.buildingsCount} / ${s.roomsCount}' : '--', 'พื้นที่ที่เปิดใช้งาน',
          Icons.apartment_rounded, SchoolAdminPalette.secondary),
      _Summary('การแจ้งเตือน', s != null ? '${s.openAlertsCount}' : '--', 'รายการที่รอตรวจสอบ',
          Icons.notifications_active_rounded, SchoolAdminPalette.red),
    ];
  ...
  Widget _reportTypes() {
    const data = [
      _Type('ภาพรวมโรงเรียน', 'นักเรียน อาคาร อุปกรณ์ ทรัพยากร และแจ้งเตือน',
          Icons.dashboard_rounded, SchoolAdminPalette.primaryDark),
      _Type('นักเรียน', 'จำนวน การเข้าเรียน สถานะบัญชี และรายการที่ควรติดตาม',
          Icons.school_rounded, Color(0xFF4F6078)),
      _Type('อุปกรณ์', 'ออนไลน์ ออฟไลน์ การตรวจความแม่นยำ และเหตุผิดปกติ',
          Icons.memory_rounded, SchoolAdminPalette.green),
      _Type('ไฟฟ้าและน้ำ', 'ปริมาณการใช้ เปรียบเทียบ และจุดที่ใช้สูงผิดปกติ',
          Icons.energy_savings_leaf_rounded, SchoolAdminPalette.secondary),
      _Type('คุณภาพอากาศ', 'PM2.5 คุณภาพอากาศ และช่วงเวลาที่ควรติดตาม',
          Icons.air_rounded, Color(0xFF71836F)),
      _Type('การแจ้งเตือน', 'เหตุการณ์ ระดับ ผู้รับผิดชอบ และผลการดำเนินการ',
          Icons.notifications_rounded, SchoolAdminPalette.red),
      _Type('ความปลอดภัย', 'การเข้าสู่ระบบผิดปกติและประวัติการเข้าถึง',
          Icons.security_rounded, Color(0xFF6C5B52)),
      _Type('อาคารและห้อง', 'จำนวนห้อง ผู้ดูแล อุปกรณ์ และสถานะพื้นที่',
          Icons.apartment_rounded, SchoolAdminPalette.primaryDark),
    ];
    return _section(
  ...
  Widget _filters() {
    final period = _Drop(
        ...
        _period,
        const ['วันนี้', '7 วันล่าสุด', 'เดือนนี้', 'เดือนที่แล้ว', 'ปีนี้'],
        (v) => setState(() => _period = v));
    final building = _Drop(
        'อาคาร',
  ...
  Widget _recentReports() {
    return _section(
      ...
      Container(
        ...
        child: const Column(
          children: [
            Icon(Icons.description_outlined, size: 36, color: SchoolAdminPalette.textSecondary),
            SizedBox(height: 8),
            Text(
...
class _Chart extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: const Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('แนวโน้มข้อมูลในช่วงที่เลือก',
                  style: TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w900,
                      color: SchoolAdminPalette.textPrimary)),
              ...
              Expanded(
                  child: CustomPaint(
                      painter: _LinePainter(), child: SizedBox.expand())),
              SizedBox(height: 8),
