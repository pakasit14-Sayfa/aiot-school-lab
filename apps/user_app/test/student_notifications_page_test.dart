...
AppNotification note(String id, String type, {bool read = false}) =>
    AppNotification(
      id: id,
      type: type,
      title: id,
...
void main() {
  testWidgets('filters display only matching canonical notifications', (
    tester,
  ) async {
  ...
  testWidgets('empty inbox contains no fabricated updates', (tester) async {
    await tester.pumpWidget(
      MaterialApp(home: NotificationsPage(load: () async => [])),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('initial read error is visible and retry loads real data', (
    tester,
  ) async {
    var fail = true;
    await tester.pumpWidget(
  ...
  testWidgets('mark failure retains unread item and exposes safe error', (
    tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets('rapid double tap performs a single mark-read mutation', (
    tester,
  ) async {
    var markCalls = 0;
  ...
  testWidgets('cached rows remain visible when a refresh fails', (
    tester,
  ) async {
    var succeed = true;
    await tester.pumpWidget(
  ...
  testWidgets(
    'a stale in-flight response is ignored once a newer load has already returned',
    (tester) async {
  ...
  testWidgets('each notification type maps to its own distinct icon', (
    tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
