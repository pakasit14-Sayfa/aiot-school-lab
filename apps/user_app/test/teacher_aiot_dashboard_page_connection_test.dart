...
void main() {
  testWidgets('zero real devices shows an honest empty state, not fabricated sensor cards', (
    tester,
  ) async {
    await _pump(tester);
    expect(find.text('ยังไม่มีอุปกรณ์ AIoT ที่ลงทะเบียนไว้ในระบบ'), findsOneWidget);
  });

  testWidgets('a real device with a real sensor reading shows the real name, not invented data', (
    tester,
  ) async {
    await _pump(
      tester,
  ...
  testWidgets('acknowledging a real alert calls the real RPC with the real alert id', (
    tester,
  ) async {
  ...
  testWidgets('a failed acknowledge shows an honest message, no leaked exception text', (
    tester,
  ) async {
    await _pump(
      tester,
