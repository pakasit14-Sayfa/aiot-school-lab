...
const artifactDir =
    '/Users/sayfa/.gemini/antigravity-cli/brain/69d565a1-b35b-4c44-84a1-f18065de1cb2';

Future<void> _capture(WidgetTester tester, String filename) async {
  await tester.runAsync(() async {
    final boundary =
        tester.renderObject(find.byKey(const Key('capture_boundary')))
    ...
    File(
      '/Users/sayfa/Downloads/school_admin_screenshots/$filename',
...
void main() {
  ...
  testWidgets('Capture School Admin Dashboard', (tester) async {
    ...
    await tester.pumpWidget(
      _wrap(const SchoolAdminDashboardPage(), size: const Size(1400, 1000)),
    );
    await tester.pump();
  ...
  testWidgets('Capture School Admin Drawer Menu', (tester) async {
    ...
    await tester.pumpWidget(
      _wrap(const SchoolAdminDashboardPage(), size: const Size(390, 844)),
    );
    await tester.pump();
        ...
        SchoolAdminIncidentInboxPage(
          initialIncidents: [
            TeacherIncidentReport(
              ...
              reason:
                  'หลอดไฟ LED ทางเดินหน้าห้อง 204 กะพริบต่อเนื่อง ขอให้ตรวจสอบและเปลี่ยนหลอดใหม่',
              severity: 'medium',
            ),
            TeacherIncidentReport(
              id: 'inc-2',
              ...
              reason:
                  'ก๊อกน้ำรั่วซึม มีน้ำหยดตลอดเวลาจากท่อใต้อ่างล้างมือ เกรงว่าจะสิ้นเปลืองน้ำประปา',
              severity: 'high',
            ),
            TeacherIncidentReport(
              id: 'inc-3',
              ...
              reason:
                  'ปลั๊กไฟห้องทดลองวิทยาศาสตร์ไม่ทำงาน ช่างเข้ามาแก้ไขระบบเบรกเกอร์เรียบร้อยแล้ว',
              severity: 'low',
            ),
          ],
        ),
