          ...
          body: Center(
            child: ElevatedButton(
              onPressed: () => Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) =>
                      TeacherIncidentDetailPage(incident: incident, actions: actions),
                ),
              ),
...
void main() {
  group('acknowledge', () {
    testWidgets('confirmed success pops only after the canonical status matches', (
      tester,
    ) async {
      var acked = false;
    ...
    testWidgets('a rejected write shows a stable failure and the page stays usable', (
      tester,
    ) async {
      final actions = _controller(
    ...
    testWidgets('the control is hidden (not just soft-blocked) while a request is pending', (
      tester,
    ) async {
  ...
  group('escalate', () {
    testWidgets('requires confirmation, then confirms success only after status becomes escalated', (
      tester,
    ) async {
      var status = 'new';
      final actions = _controller(
      ...
      expect(
        ...
        findsNothing,
        reason: 'the confirmation dialog must appear before any write happens',
      );
      ...
      expect(find.text('open detail'), findsOneWidget);
      expect(find.text('ยกระดับเป็นเหตุฉุกเฉินเรียบร้อยแล้ว'), findsOneWidget);
    });

    testWidgets('cancelling the confirmation dialog never calls the write', (tester) async {
      var writes = 0;
    ...
    testWidgets('a rejected escalate shows a stable failure, distinct from unconfirmed', (
      tester,
    ) async {
      final actions = _controller(
  ...
  group('close', () {
    testWidgets('the chosen resolution reaches the RPC unchanged and confirms on exact match', (
      tester,
    ) async {
    ...
    testWidgets('a blank note is blocked before any write, dialog stays open', (
      ...
      expect(writes, 0);
      expect(find.text('ยืนยันปิดเหตุ'), findsOneWidget, reason: 'dialog is still open');
    });

    testWidgets('a failed close keeps the dialog open with the typed note retained', (
      tester,
    ) async {
      final actions = _controller(
  ...
  group('save note', () {
    testWidgets('confirms and clears the field only when the canonical read matches', (
      tester,
    ) async {
      String? saved;
      final actions = _controller(
      ...
      await tester.enterText(find.byType(TextField), 'ตรวจสอบที่เกิดเหตุแล้ว');
    ...
    testWidgets('a blank note cannot be submitted', (tester) async {
      ...
      expect(writes, 0);
      expect(find.text('open detail'), findsNothing, reason: 'nothing happened, page stays open');
    });

    testWidgets('a rejected save shows a safe error and the typed text is retained', (
      tester,
    ) async {
      final actions = _controller(
        saveIncidentNote: (_, __) async => throw StateError('secret SQL detail'),
      );
      await _pumpPushed(tester, _incident(status: 'in_progress'), actions);
    ...
    testWidgets('a canonical mismatch after a successful write is unconfirmed, text retained', (
      tester,
    ) async {
      final actions = _controller(
  ...
  group('terminal state', () {
    testWidgets('a resolved incident shows no action buttons or note editor', (tester) async {
      final actions = _controller();
