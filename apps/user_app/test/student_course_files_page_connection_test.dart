...
void main() {
  testWidgets(
    'a real course with a real file shows the real subject/file name, not fabricated content',
    (tester) async {
  ...
  testWidgets(
    'opening a non-image file resolves the real signed URL for the real file id',
    (tester) async {
      String? requestedId;
      await tester.pumpWidget(
        MaterialApp(
  ...
  testWidgets(
    'opening an image previews it via the real signed URL, not a fake path',
    (tester) async {
      String? requestedId;
      await tester.pumpWidget(
        MaterialApp(
